...
| Path | What |
|---|---|
| `/data/home/$USER/workspace/orbit/` | This repo |
| `.../orbit/thirdparty/Megatron-LM/` | `radixark/Megatron-LM @ miles-main` — git submodule, pinned in `.gitmodules` |
| `.../orbit/thirdparty/sglang/` | `sgl-project/sglang @ sglang-miles` — git submodule, pinned in `.gitmodules` |
| `.../orbit/runs/<name>/<YYMMDD_HHMMSS>/` | per-run dir: run.log, args.json, ray_*.log, MANIFEST.json (gitignored) |
| `.../orbit/checkpoints/<name>/` | `--save` / `--load` target (gitignored) |
| `/data/shared/conda/miniconda3/envs/orbit/` | Conda env |
...
- `scripts/experiments/<name>.sh` — **pure config**, orchestrator-agnostic. Declares asset metadata (`HF_MODEL_REPO`, `HF_DATASETS`, `HF_*_DIR` paths), resource metadata (`EXPERIMENT_NODES`, `EXPERIMENT_TIME`), and the `ORBIT_ARGS` bash array. **No side effects when sourced** — no `ray start`, no `sbatch`, no `hf download`. Could be sourced by a local runner, a k8s wrapper, anything.
- `scripts/slurm/submit.sh` — the slurm wrapper. Takes an experiment name, sources the recipe, downloads missing assets, calls `sbatch ... launch_orbit.sbatch`. The only file you run by hand.
...
2. Activates the `orbit` conda env.
3. Sources `scripts/experiments/qwen3-4B-disagg-2node.sh` to pick up `ORBIT_ARGS`, `HF_*` metadata, and `EXPERIMENT_NODES/TIME`.
4. Idempotently `hf download`s `Qwen/Qwen3-4B`, `DAPO-Math-17K`, `aime-2024` into `/data/shared/hf_cache/{models,data}/...` if missing.
5. `exec sbatch ... --export=ALL,RECIPE=<abs path to scripts/experiments/<name>.sh> scripts/slurm/launch_orbit.sbatch`. (The torch_dist artifact is NOT checked here — `launch_orbit.sbatch` auto-converts on the head node if missing.)

`launch_orbit.sbatch` then:
1. **Activates** the `orbit` conda env on the controller process.
...
4. **Source recipe early** — sources `$RECIPE` to bring `ORBIT_ARGS`, `HF_TORCHDIST_DIR`, `MODEL_ARGS` into the controller's scope. Dumps args to `<run_dir>/args.json`.
...
8. **Submit** — runs `ray job submit … -- python3 train.py "${ORBIT_ARGS[@]}"` from the head node (the recipe was already sourced in step 4).
...
- Experiment file uses `$ORBIT_REPO/checkpoints/$RUN_NAME` for both `--load` and `--save`.
- `--save-interval 20` writes a Megatron `torch_dist` checkpoint every 20 rollouts + updates `latest_checkpointed_iteration.txt`.
- On slurm requeue (SIGTERM caught by `--signal=B:SIGTERM@120`, node death, preempt), the launcher re-runs from scratch — boots a fresh cluster, resubmits with the same args. Orbit reads `latest_checkpointed_iteration.txt` from `--load` and continues from that step.
...
| **SGLang health** (orbit) | `/health_generate` fails | `ray.kill` the engine; rollout continues with remaining engines. Triggered by `--use-fault-tolerance`. |
...
JOBID=<from squeue>
NAME=<job-name>
RUN_DIR=/data/home/$USER/workspace/orbit/runs/$NAME/j$JOBID
...
**Recovery**: re-run `install_env.sh`. If still failing, conda-install matching CUDA toolkit: `conda install -n orbit -c nvidia cuda-toolkit=12.8` (heavy, ~3GB).
...
| Path | Purpose |
|---|---|
| `scripts/experiments/<name>.sh` | One per experiment — **pure config**, orchestrator-agnostic. Defines `ORBIT_ARGS` + asset & resource metadata. |
...
| `scripts/slurm/launch_orbit.sbatch` | Sbatch entry — healthcheck + ray + submit + teardown (invoked by `submit.sh`). |
| `scripts/slurm/setup/README.md` | First-time install walkthrough (read once). |
| `scripts/slurm/setup/install_env.sh` | One-time: build the `orbit` conda env via uv. |
| `scripts/slurm/setup/convert_checkpoint.sh` | Optional: manual HF → Megatron `torch_dist` (used for large multi-node converts). |
| `orbit/utils/health_monitor.py` | `RolloutHealthMonitor` — activated by `--use-fault-tolerance`. |
| `orbit/ray/placement_group.py` | PACK + (node_ip, gpu_id) sort that pins train-on-head, rollout-on-worker. |
