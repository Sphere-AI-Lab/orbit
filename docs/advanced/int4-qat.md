...
orbit INT4 QAT keeps actor parameters in Megatron's training dtype (BF16 by
...
| Component | Role |
|---|---|
| Hugging Face checkpoint | `--hf-checkpoint` initializes SGLang and defines the rollout storage format. The generic orbit exporter reads its quantization config when packing live updates. |
...
| Megatron Bridge or the orbit raw-mode weight exporter | Maps updated Megatron weights to Hugging Face names. For an INT4 rollout checkpoint, the Kimi Bridge returns packed tensors itself; the generic orbit exporter packs eligible `.weight` tensors. |
...
3. During training, Megatron fake-quantizes every routed-expert
   ...
   trainable weight.
4. At a weight-update boundary, Megatron Bridge or the orbit raw-mode exporter
   exports Hugging Face-named tensors. The Kimi-K2.5 Bridge returns routed
   experts already packed as INT4; the generic orbit path packs them from the
...
- Tensor scope: to match the current Megatron hook, pack the routed expert
  ...
  checkpoint dtype.
- Shape: the generic orbit packer and direct converter require the final
...
When a fresh run has no usable `--load` checkpoint and no `--ref-load`, orbit
...
ORBIT_SCRIPT_EXTERNAL_RAY=1 python scripts/run_kimi_k25.py train \
  --model-name Kimi-K2.5 \
  --num-nodes 32
```
...
The orbit direct converter creates a symmetric, group-wise INT4 checkpoint
...
CUDA and the `fake_int4_quant_cuda` extension installed by the orbit image.
...
| The converter cannot import `fake_int4_quant_cuda` | Use the orbit CUDA image or build the repository's INT4 QAT extension. |

## Hardware and test coverage

The direct converter and generic orbit INT4 packer use the CUDA
