...
**SGLang side.** Orbit enables `enable_return_routed_experts` automatically when
...
results in `sample.rollout_routed_experts` (`orbit/utils/types.py`). The
trainer pushes the arrays through `RoutingReplayManager`
(`orbit/utils/replay_base.py`), and `replay_utils.py` plugs them into the
...
`orbit/utils/types.py`). For a 32K-token sequence, 60 layers, and
