...
| `WHEELS_TAG_X86` / `WHEELS_TAG_ARM64`                                                                  | the two **complete** rolling wheels release tags, selected by CUDA and architecture (`cu130-x86_64` / `cu130-aarch64`; cu12-x86 overrides with `cu129-x86_64`). In a multi-arch build the Dockerfile **picks one by `TARGETARCH`** and installs it **verbatim**; CI fingerprints the asset list because binaries can be replaced without changing the tag. |
