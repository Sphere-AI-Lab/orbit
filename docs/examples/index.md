...
description: "Launchable training recipes:"
...
- **[adapter_first](/examples/adapter-first)**: Adapter-first RL recipes (OFT/LoRA phase-0 suites and OPD cost studies).
- **[high_precision](/examples/high-precision)**: BF16 full-finetune and PEFT RL recipes, including the one-trunk adapter-critic PPO smokes.
- **[infra_features](/examples/infra-features)**: Infrastructure feature demos (async modes, low precision, weight transfer).
- **[infra_features/fully_async](/examples/infra-features/fully-async)**: Fully asynchronous rollout/training overlap recipes.
- **[infra_features/low_precision](/examples/infra-features/low-precision)**: RL on quantized checkpoints (INT4/NVFP4/FP8/MXFP4) with OFT.
- **[infra_features/p2p_weight_transfer](/examples/infra-features/p2p-weight-transfer)**: Peer-to-peer weight-update transfer mode.
- **[infra_features/random_async](/examples/infra-features/random-async)**: Randomized async off-policy stress recipes.
- **[multi_lora](/examples/multi-lora)**: Multi-tenant LoRA training and rollout.
- **[on_policy_distillation](/examples/on-policy-distillation)**: On-policy distillation (MOPD) recipes: external/managed teachers, teacher pools, self-teacher.
- **[on_policy_distillation/qwen3_5_35b_selfdistill](/examples/on-policy-distillation/qwen3-5-35b-selfdistill)**: Qwen3.5-35B self-distillation campaign.
- **[peft_arena/backend](/examples/peft-arena/backend)**: PEFT-Arena evaluation backend and graders.
- **[peft_arena/eval](/examples/peft-arena/eval)**: PEFT-Arena math evaluation launchers.
- **[ppo](/examples/ppo)**: PPO recipes on the vendored upstream flow.
- **[search_r1](/examples/search-r1)**: Search-R1 tool-use RL recipes.
- **[sft](/examples/sft)**: Supervised fine-tuning launchers.
- **[swe-agent-harbor-docker](/examples/swe-agent-harbor-docker)**: SWE-agent Harbor docker harness.
- **[tau_bench](/examples/tau-bench)**: Tau-bench agentic RL recipes.
