...
4. **Quantized trunk + adapter critic is rejected** (`orbit/peft/megatron/low_precision_bootstrap.py:151-156`): one-trunk aliasing shares `Parameter`s only; quantized trunk weights/scales live in checkpoint-created buffers. INT4/FP4 one-trunk PPO (P1-INT4, X2-PPO) is blocked until this is lifted; the BF16 feasibility frontier is runnable today.
