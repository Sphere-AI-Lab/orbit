#!/usr/bin/env python3
"""Download and process data to Orbit format."""
...
def convert_to_orbit_format(
    input_path: str, output_path: str, limit: int = None, split: str = "train", subset: str = "gym"
):
    """Convert JSONL to Orbit format.

    Args:
        input_path: Path to input JSONL file
        output_path: Path to output JSONL file in Orbit format
...
def main():
    parser = argparse.ArgumentParser(description="Download HuggingFace dataset and convert to Orbit format")
    ...
    if input_path.exists() and input_path.suffix == ".jsonl":
        ...
        convert_to_orbit_format(args.input, args.output, args.limit, args.split, args.subset)
    else:
        ...
        try:
            ...
            convert_to_orbit_format(tmp_path, args.output, args.limit, split=args.split, subset=args.subset)
        finally:
