#!/usr/bin/env bash
# Qwen2.5-0.5B-Instruct BF16 FULL fine-tuning on the math dataset — async training mode.
# Mechanical copy of run-qwen2_5-0_5b-bf16-math-oft.sh with the PEFT flags removed: no
# adapter, so update_weights ships the full model to the rollout engines (the legacy
...
# === Recipe identity ===
LAUNCHER_NAME=run_qwen25_05b_bf16_math_megatron_fullft_async
...
SAVE_DIR="${ORBIT_ROOT}/orbit_ckpts/Qwen2.5-0.5B-Instruct_math_fullft_async"
: "${TRAIN_JSONL:?set TRAIN_JSONL to a training jsonl path}"
TEST_JSONL=${TEST_JSONL:-}

# === Resources: 1 actor GPU + 1 disjoint rollout GPU ===
