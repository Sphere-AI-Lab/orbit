#!/usr/bin/env bash
# Qwen2.5-3B-Instruct BF16 Canonical-OFT adapter with GRPO on the OpenR1 math set.
...
# factors under study change: full FT (no PEFT adapter) and GRPO (group-relative
# advantages over the same 4 samples/prompt; no critic, no KL — the benchmark's
# PPO runs were also KL-free). Topology: 1 actor + 3 rollout GPUs (no critic
# worker, no idle GPU).
set -euo pipefail
...
# === Recipe identity ===
SEED="${SEED:-1234}"
LAUNCHER_NAME=run_qwen25_3b_bf16_math_oft_grpo_seed${SEED}
...
SAVE_ROOT="${SAVE_ROOT:-${ORBIT_ROOT}/orbit_ckpts/oft_grpo}"
SAVE_DIR="${SAVE_DIR:-${SAVE_ROOT}/Qwen2.5-3B-Instruct_oft_grpo_seed${SEED}}"
