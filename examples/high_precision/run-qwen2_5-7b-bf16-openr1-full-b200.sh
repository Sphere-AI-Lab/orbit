#!/usr/bin/env bash
# Qwen2.5-7B BF16 full FT on PEFT-Arena openr1-50k. B200 variant of
# run-qwen2_5-7b-bf16-openr1-full.sh.
#
# Recipe (full FT, lr, batch sizes, num-rollout, n-samples, context lengths,
# KL/clip) is IDENTICAL to the base script -- only Blackwell perf/memory knobs
# change to use B200's 192 GB HBM:
#   * max-tokens-per-gpu 16384 -> 32768
#   * keep activation recompute on for full-FT forward/backward headroom
#   * keep Adam on GPU; Miles Qwen docs remove CPU Adam on Blackwell paths
#   * offload rollout before train; resident SGLang leaves too little full-FT
#     backward headroom on long-response batches
#   * raise SGLang KV/request/cuda-graph budgets to the existing B200 PEFT recipe
set -euo pipefail
...
# === Recipe identity ===
LAUNCHER_NAME=run_qwen25_7b_openr1_full_bs32_r1000_lr1e6_b200
