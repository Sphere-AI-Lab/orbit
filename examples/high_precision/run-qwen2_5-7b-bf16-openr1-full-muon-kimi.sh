#!/usr/bin/env bash
# Qwen2.5-7B BF16 full FT on PEFT-Arena openr1-50k, Muon-Kimi optimizer.
# Kimi-Muon = orbit's Muon configured per Moonshot arXiv:2502.16982
# (see examples/optimizers/muon-kimi.env). Requires the emerging-optimizers
# package — see README "Optional: Muon / Pion optimizers". Self-contained.
set -euo pipefail
...
# === Recipe identity ===
LAUNCHER_NAME=run_qwen25_7b_openr1_full_muon_kimi_bs32_r1000_lr1e6
...
SAVE_DIR="${SAVE_DIR:-${ORBIT_ROOT}/orbit_ckpts/exp_ckpt/${LAUNCHER_NAME#run_}_modelonly_$(date +%Y%m%d_%H%M%S)}"
...
OPTIMIZER_ARGS=(
    # Muon-Kimi preset (see examples/optimizers/muon-kimi.env). Kimi RMS-matches
    # Muon's update to AdamW, so the AdamW learning rate transfers as-is.
    --optimizer muon
    --lr 1e-6
    --lr-decay-style constant
    --weight-decay 0.0
    --muon-scale-mode spectral          # sqrt(max(fan_out, fan_in)) update scale
    --muon-extra-scale-factor 0.2       # Kimi's 0.2 -> 0.2*sqrt(max(A,B))
    --muon-nesterov
    --muon-coefficient-type simple      # Keller-Jordan/Kimi NS coeffs, NOT default "quintic"
    --muon-num-ns-steps 5
    --muon-momentum 0.95
    --muon-scalar-optimizer adam        # AdamW path for 1-D params (embeds/norms)
)
# NOTE: use_distributed_optimizer is auto-disabled for muon (own sharding);
# no flag needed. Muon runs under this launcher's tensor/context parallelism.
