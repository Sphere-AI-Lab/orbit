#!/usr/bin/env bash
# Qwen2.5-7B BF16 + OFT (block-size 32) on PEFT-Arena openr1-50k with active
# KL loss. B200 variant of run-qwen2_5-7b-bf16-openr1-oft-b32-kl.sh.
#
# Recipe is the H100 KL script plus the same B200 perf/memory knobs as the
# non-KL B200 launcher. Because this is PEFT/OFT, do not pass --ref-load:
# Orbit computes the reference policy by disabling the adapter at ref-forward
# time.
set -euo pipefail
...
# === Recipe identity ===
LAUNCHER_NAME=run_qwen25_7b_openr1_oft_b32_bs32_r1000_lr1e5_kl_b200
