#!/usr/bin/env bash
# Qwen3-30B-A3B (MoE) BF16 + full FT on PEFT-Arena openr1-50k, tuned for 8xB200.
# TP=4 EP=4, TRT-LLM MHA, CUDA graph, no optimizer CPU offload, 500 rollouts.
# Variant: LR=1e-6, weight-decay=0.0, wider SGLang KV pool (524k tokens, 64 requests).
...
# === Recipe identity ===
LAUNCHER_NAME=run_qwen3_30b_a3b_openr1_full_tp4_ep4_bs16_r1000_lr1e6_kl
...
# Stripe the full-FT checkpoint directory across 8 OSTs (16 MiB chunks) so
# Megatron's distributed save is not bottlenecked by a single OST.
SAVE_DIR_STRIPE_COUNT=8
SAVE_DIR_STRIPE_SIZE=16M
