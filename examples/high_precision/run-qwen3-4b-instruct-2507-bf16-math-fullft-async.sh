#!/usr/bin/env bash
# Qwen3-4B-Instruct-2507 BF16 FULL fine-tuning on the math dataset — async training mode.
# Mechanical copy of run-qwen3-4b-instruct-2507-bf16-math-oft-async.sh with the PEFT
# flags removed: no adapter, so update_weights ships the full model to the rollout
# engines (the legacy full-parameter sync path used when the PEFT method is none).
...
# === Recipe identity ===
LAUNCHER_NAME=run_qwen3_4b_bf16_math_megatron_fullft_async
...
SAVE_DIR="${ORBIT_ROOT}/orbit_ckpts/Qwen3-4B-Instruct-2507-BF16_math_fullft_async"
...
OPTIMIZER_ARGS=(
    ...
    # give the full-FT arm its own learning rate and a per-repeat seed.
    --lr "${LR:-3e-6}"
