#!/usr/bin/env bash
# Kimi-K2.6 W4A16 INT4 + OFT on PEFT-Arena openr1-50k with CUDA 13.2, CP=2, and 4096 response len. Self-contained launcher.
#
# CUDA 13.2 K2.6 variant of the PEFT-Arena OpenR1 launcher. It keeps the
# Kimi CP=2 4096-token recipe self-contained rather than delegating to another
# launch script.
set -euo pipefail
...
# === Recipe identity ===
LAUNCHER_NAME=${LAUNCHER_NAME:-run_kimi_k26_int4_peft_arena_openr1_oft_cp2_cuda132_4096}
...
SAVE_DIR=${SAVE_DIR:-${ORBIT_ROOT}/orbit_ckpts/Kimi-K2.6-W4A16_${DATASET}_oft}
...
SGLANG_OFT_PARITY_MODE=${SGLANG_OFT_PARITY_MODE:-0}
...
WANDB_GROUP=${WANDB_GROUP:-orbit-kimi-k26-int4-${DATASET}-oft}
...
if is_true "${SGLANG_OFT_PARITY_MODE}"; then
