#!/usr/bin/env bash
# Qwen2.5-0.5B-Instruct BF16 LoRA pure MOPD with an adapter-swap teacher
# (--opd-teacher adapter:<path>, megatron mode). Smoke config for the teacher-as-
# adapter-slot path: the teacher is a checkpoint's adapter weights loaded on top
# of the frozen base, so there is no second model load. This exercises the
...
# === Recipe identity ===
LAUNCHER_NAME=smoke_qwen25_05b_opd_adapter_swap
