#!/usr/bin/env bash
# Qwen2.5-0.5B-Instruct BF16 LoRA pure MOPD with the free trainer-side teacher
# (--opd-teacher base, megatron mode). Smoke config for the teacher-as-
# adapter-slot path: the teacher is the student's own frozen base (adapter
# toggled off for the teacher forward), so there is no second model load and
# no teacher checkpoint. With no KL configured here, this exercises the
# adapter_off plan specifically (with --kl-coef on, the ref forward would be
# reused instead: the teacher is literally free either way).
...
# === Recipe identity ===
LAUNCHER_NAME=smoke_qwen25_05b_opd_free_teacher
