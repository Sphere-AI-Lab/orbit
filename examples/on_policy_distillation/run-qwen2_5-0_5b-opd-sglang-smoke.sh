#!/usr/bin/env bash
# Qwen2.5-0.5B-Instruct BF16 full-finetune pure MOPD (external SGLang teacher).
# Smoke config for the sglang-teacher scoring paths against a real (different)
# teacher server. OPD_TOPK=0 exercises sampled-token teacher_log_probs;
# OPD_TOPK>0 exercises the top-k distributional reverse KL (opd_reverse_kl),
# optionally with OPD_TAIL_BUCKET=1 for the exact k+1-bucket estimate.
# Requires a running teacher: point OPD_TEACHER_URL at its /generate endpoint.
set -euo pipefail
...
# === Recipe identity ===
LAUNCHER_NAME=smoke_qwen25_05b_opd_sglang
