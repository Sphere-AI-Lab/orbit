...
import hashlib
from sglang_adapter_utils import build_sglang_engine_config
...
@dataclass
class _VllmShim:
    """Mimics vllm.RequestOutput shape so the existing post-processing code
    does not need to fork by backend.
    """
    outputs: list
    finished: bool = True

    @classmethod
    def from_sglang(cls, item: dict) -> "_VllmShim":
        text = item["text"] if isinstance(item, dict) else getattr(item, "text", "")
        meta = item.get("meta_info", {}) if isinstance(item, dict) else {}
        finish_reason = meta.get("finish_reason", {}).get("type") if isinstance(meta.get("finish_reason"), dict) else meta.get("finish_reason")
        return cls(outputs=[_SglOutput(text=text, finish_reason=finish_reason)])
...
def maybe_merge_lora_weights_and_save(model_name_or_path: str) -> str | None:
    if is_adapter_model(model_name_or_path):
        ...
        model.config._name_or_path = base_model_name_or_path
        model.config.torch_dtype = next(model.parameters()).dtype
        ...
        try:
            slow_tokenizer = AutoTokenizer.from_pretrained(base_model_name_or_path, use_fast=False)
            slow_tokenizer.save_pretrained(temp_dir, legacy_format=True)
            tokenizer_saved = True
        except Exception:
            pass

        try:
            fast_tokenizer = AutoTokenizer.from_pretrained(base_model_name_or_path)
            fast_tokenizer.save_pretrained(temp_dir)
            tokenizer_saved = True
        except Exception:
            pass
        ...
        if os.path.exists(base_model_name_or_path):
            ...
            for name in artifact_names:
                source_path = Path(base_model_name_or_path) / name
        ...
        if not tokenizer_saved and not (Path(temp_dir) / "tokenizer_config.json").exists():
            raise RuntimeError(f"Failed to save tokenizer assets for merged adapter at {temp_dir}")
    else:
        temp_dir = None
...
def parse_args():
    ...
    parser.add_argument("--use_sglang", action="store_true",
                        help="Use SGLang offline engine for generation (mutually exclusive with --use_vllm)")
    parser.add_argument("--num_gpus", default=1, type=int,
                        help="Number of GPUs for the inference backend (vLLM tensor_parallel_size or SGLang tp_size)")
    parser.add_argument("--save_outputs", action="store_true")
    parser.add_argument("--overwrite", action="store_true")
    parser.add_argument("--use_safetensors", action="store_true")
    parser.add_argument("--dtype", default="bfloat16", type=str)
    parser.add_argument("--gpu_memory_utilization", default=0.7, type=float)
    parser.add_argument("--max_model_len", default=9216, type=int)
    parser.add_argument("--lora_weights", default=None, type=str, help="Path or repo id for LoRA weights (PEFT)")
    parser.add_argument("--adapter_path", default=None, type=str, help="Direct SGLang PEFT adapter path")
    parser.add_argument("--adapter_type", default=None, type=str, choices=["LORA", "OFT", "lora", "oft"])
    parser.add_argument("--adapter_name", default="orbit_eval_adapter", type=str)
    parser.add_argument("--lora_backend", default="triton", type=str)
    parser.add_argument("--oft_backend", default="triton", type=str)
    ...
    if args.use_vllm and args.use_sglang:
        parser.error("--use_vllm and --use_sglang are mutually exclusive")
    args.top_p = (
...
def setup(args):
    ...
    if args.use_vllm:
        temp_dir_for_merged_weights = maybe_merge_lora_weights_and_save(args.model_name_or_path)
        delete_model_after_job_finished = False
        model_name_or_path = args.model_name_or_path
        if temp_dir_for_merged_weights is not None:
            print(f'saving merged model to {temp_dir_for_merged_weights}, will delete after eval')
        ...
        delete_model_after_job_finished = False
        engine_kwargs, sglang_generate_kwargs, tokenizer_path = build_sglang_engine_config(args)
        llm = sgl.Engine(**engine_kwargs)
        args._sglang_generate_kwargs = sglang_generate_kwargs
        tokenizer = None
        if args.apply_chat_template:
            tokenizer = AutoTokenizer.from_pretrained(
    ...
    if delete_model_after_job_finished:
        assert model_name_or_path != '/'
        print(f'deleting {model_name_or_path}')
        os.system(f"rm -rf {model_name_or_path}")
...
def main(llm, tokenizer, data_name, args):
    ...
    elif "llama" in args.prompt_type:
    ...
    for epoch in range(max_func_call):
        ...
        if args.use_vllm:
            ...
            sglang_generate_kwargs = getattr(args, "_sglang_generate_kwargs", {})
            sgl_outputs = llm.generate(
                prompts,
                sampling_params={
                    "temperature": args.temperature,
                    "top_p": args.top_p,
                ...
                **sglang_generate_kwargs,
            )
            shimmed = [_VllmShim.from_sglang(item).outputs[0] for item in sgl_outputs]
            outputs = [(o.text, o.finish_reason) for o in shimmed]
        else:
    ...
    for i, sample in enumerate(samples):
        ...
        for j in range(len(preds)):
            if sample["gt"] in ["A", "B", "C", "D", "E"] and preds[j] not in [
                ...
                preds[j] = "".join(
                    [c for c in (preds[j] or [])if c in ["A", "B", "C", "D", "E"]]
                )
