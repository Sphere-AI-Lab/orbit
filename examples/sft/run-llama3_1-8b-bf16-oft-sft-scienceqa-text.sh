#!/usr/bin/env bash
# Llama-3.1-8B OFT SFT on text-only derek-thomas/ScienceQA converted with tools/convert_sft_dataset_to_orbit.py.
set -euo pipefail
...
SFT_DATASET_NAME="scienceqa-text"
SFT_TRAIN_JSONL_DEFAULT="${SFT_DATA_ROOT}/scienceqa-text/train.jsonl"
SFT_SAVE_DIR_SUFFIX="scienceqa_text"
SFT_TOTAL_EPOCHS="${SFT_TOTAL_EPOCHS:-3}"

SFT_DATASET_SAFE="${SFT_DATASET_NAME//[^a-zA-Z0-9]/_}"

# === Recipe identity ===
LAUNCHER_NAME=${LAUNCHER_NAME:-run_llama31_8b_bf16_oft_sft_${SFT_DATASET_SAFE}}
...
SAVE_DIR="${SAVE_DIR:-${ORBIT_ROOT}/orbit_ckpts/Llama-3.1-8B_oft_sft_${SFT_SAVE_DIR_SUFFIX}}"
...
ROLLOUT_BATCH_SIZE="${ROLLOUT_BATCH_SIZE:-${SFT_ROLLOUT_BATCH_SIZE:-16}}"
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-${SFT_GLOBAL_BATCH_SIZE:-8}}"
...
ROLLOUT_ARGS=(
    ...
    --loss-mask-type "${LOSS_MASK_TYPE:-response_only}"
