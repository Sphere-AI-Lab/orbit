...
_PRELOADED_CHECKPOINT_IDENTITY_ATTRS = (
    "_miles_loaded_dist_checkpoint_path",
    "_miles_loaded_dist_checkpoint_prefix",
    "_miles_restored_modelopt_checkpoint_path",
)
...
def _propagate_preloaded_checkpoint_identity(source_chunks, transformed_chunks) -> None:
    ...
    copying Miles's load identity, the ordinary initialization path loads the
...
def _adapter_debug_names(module_name: str, module) -> list[str]:
    ...
    for attr_name, leaf_name in _CANONICAL_OFT_ADAPTER_LEAVES.items():
        ...
        if split_adapter is None:
            continue
        names.append(getattr(split_adapter, "_oft_debug_name", None) or _replace_module_leaf(module_name, leaf_name))
    return names
...
def _assert_peft_wrapped_modules(
    ...
    *,
    ...
    include_dsv4_native_moe_oft = bool(
        requested_target_modules and any(_target_is_dsv4_native_moe_oft(target) for target in requested_target_modules)
    )
    ...
    if requested_target_modules:
        missing_targets = [
            ...
            if summary["targets"].get(target, 0) == 0 and summary["targets"].get(_target_leaf_name(target), 0) == 0
        ]
        if missing_targets:
            raise RuntimeError(
...
def _make_peft_pre_wrap_hook(
    ...
    *,
    ...
    def hook(model_chunks):
        ...
        from miles.audit.peft_wrap import dump_megatron_audit
