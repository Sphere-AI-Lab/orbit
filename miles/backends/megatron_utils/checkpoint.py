...
_MILES_TRAINING_CHECKPOINT_MARKER = ".miles_training_checkpoint.json"
_MILES_TRAINING_CHECKPOINT_FORMAT = "miles.training_checkpoint"
_MILES_TRAINING_CHECKPOINT_VERSION = 1
...
def _write_miles_training_checkpoint_marker(
    save_root: str | Path,
    iteration: int,
    role: str,
    *,
    ...
    if not _bounded_nonnegative_integer(iteration):
        raise ValueError(f"invalid Miles checkpoint iteration: {iteration!r}")
    if role not in _CHECKPOINT_ROLES:
        raise ValueError(f"invalid Miles checkpoint role: {role!r}")

    checkpoint_dir = _checkpoint_iteration_dir(save_root, iteration, release=release)
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    marker_path = checkpoint_dir / _MILES_TRAINING_CHECKPOINT_MARKER
    marker = {
        "format": _MILES_TRAINING_CHECKPOINT_FORMAT,
        "version": _MILES_TRAINING_CHECKPOINT_VERSION,
        "iteration": iteration,
        "role": role,
...
def save_checkpoint(iteration, model, optimizer, opt_param_scheduler, *args, **kwargs):
    """Save through Megatron and mark the result as an Miles training checkpoint.
    ...
    if is_rank_zero and role is not None and getattr(runtime_args, "save", None) is not None:
        ...
        marker_path = _write_miles_training_checkpoint_marker(
            runtime_args.save,
            iteration,
            role,
        ...
        logger.info("Wrote Miles training checkpoint marker to %s", marker_path)
    ...
    if checkpoint_step is not None:
        ...
        if (load_path / ".metadata").is_file():
            ...
            if direct_iteration is not None:
                ...
                marker = _read_miles_training_checkpoint_marker(candidate)
                if marker is None:
                    raise ValueError(
...
def _raise_if_incomplete_direct_distributed_checkpoint(load_path: str | Path) -> None:
    ...
    marker_exists = (resolved_path / _MILES_TRAINING_CHECKPOINT_MARKER).is_file()
...
def _read_miles_training_checkpoint_marker(checkpoint_dir: Path) -> dict | None:
    marker_path = checkpoint_dir / _MILES_TRAINING_CHECKPOINT_MARKER
    ...
    try:
        ...
        raise RuntimeError(f"invalid Miles training checkpoint marker at {marker_path}") from exc

    if type(marker) is not dict:
        raise RuntimeError(f"invalid Miles training checkpoint marker at {marker_path}: expected an object")
    if marker.get("format") != _MILES_TRAINING_CHECKPOINT_FORMAT:
        raise RuntimeError(f"invalid Miles training checkpoint marker format at {marker_path}")
    if marker.get("version") != _MILES_TRAINING_CHECKPOINT_VERSION:
        raise RuntimeError(
            f"unsupported Miles training checkpoint marker version at {marker_path}: {marker.get('version')!r}"
        )
    if marker.get("role") not in _CHECKPOINT_ROLES:
        raise RuntimeError(f"invalid Miles training checkpoint role at {marker_path}: {marker.get('role')!r}")
    if not _bounded_nonnegative_integer(marker.get("iteration")):
        raise RuntimeError(f"invalid Miles training checkpoint iteration at {marker_path}")
    for state_field in ("optimizer_state_saved", "scheduler_state_saved"):
        if type(marker.get(state_field)) is not bool:
            raise RuntimeError(f"invalid Miles training checkpoint {state_field} at {marker_path}")
    ...
    if iteration_match is not None and marker["iteration"] != int(iteration_match.group(1)):
        raise RuntimeError(
            f"Miles training checkpoint marker iteration mismatch at {marker_path}: marker={marker['iteration']}, directory={checkpoint_dir.name}"
        )
    return marker
...
def _select_megatron_training_checkpoint(
    ...
    # These flags request a weights-only warm start. Keep Miles's model-only
    ...
    marker = _read_miles_training_checkpoint_marker(checkpoint_dir)
    if marker is not None:
        if marker["role"] != expected_role:
            logger.info(
                "Treating Miles %s checkpoint at %s as model-only for %s bootstrap",
                marker["role"],
                checkpoint_dir,
                expected_role,
            )
        ...
        if not (marker["optimizer_state_saved"] and marker["scheduler_state_saved"]):
            raise RuntimeError(
                f"Miles {expected_role} checkpoint at {checkpoint_dir} was saved without complete optimizer/scheduler state and cannot resume training. Use --no-load-optim for an explicit model-only warm start, or resume from a checkpoint saved without --no-save-optim."
            )
    ...
    if inferred_role is not None and inferred_role != expected_role:
        logger.info(
            "Treating legacy Miles %s checkpoint at %s as model-only for %s bootstrap",
            inferred_role,
            checkpoint_dir,
            expected_role,
        )
    ...
    if inferred_role is None and expected_role == "critic":
        logger.warning(
            "Legacy distributed checkpoint at %s has training state but no defensible role metadata; using model-only critic bootstrap. Re-save it with an Miles checkpoint marker to enable critic resume.",
            checkpoint_dir,
        )
        return None

    logger.warning(
        "Resuming %s from legacy distributed checkpoint training state at %s; future saves will include an explicit Miles marker.",
        expected_role,
        checkpoint_dir,
    )
    return checkpoint_dir
...
def _load_selected_megatron_training_checkpoint(args, checkpoint_dir: Path, **load_kwargs):
    ...
    if iteration_match is not None:
        ...
        if marker is None:
            raise RuntimeError(
                f"Miles training checkpoint iteration cannot be derived from directory or marker at {checkpoint_dir}"
            )
    ...
    with tempfile.TemporaryDirectory(prefix="miles-megatron-load-") as temporary_root:
...
def _legacy_checkpoint_consensus_group():
    """Use Miles's world Gloo group, or the default group in Gloo-only tests."""
    ...
    if str(dist.get_backend()).lower().endswith("gloo"):
        return None
    raise RuntimeError("legacy Megatron checkpoint preflight requires Miles's world Gloo process group")
...
def _local_legacy_megatron_preflight(
    ...
    *,
    ...
    marker = _read_miles_training_checkpoint_marker(checkpoint_dir) if checkpoint_dir is not None else None

    if marker is not None and selected_iteration is not None and marker["iteration"] != selected_iteration:
        raise RuntimeError(
            f"Miles training checkpoint iteration mismatch at {checkpoint_dir}: "
    ...
    if marker is not None and selected_iteration is not None and load_training_state:
        if expected_role is not None and marker["role"] != expected_role:
            raise RuntimeError(
                f"Miles checkpoint role mismatch at {checkpoint_dir}: "
        ...
        if not explicit_model_only and not (marker["optimizer_state_saved"] and marker["scheduler_state_saved"]):
            raise RuntimeError(
                f"Miles {marker['role']} checkpoint at {checkpoint_dir} was saved without complete "
...
def load_checkpoint_orbit(
    ...
    *,
    ...
    into miles' actor (follow-up); miles' primary loader is load_checkpoint below.
    ...
    restored together; model-only sources retain Miles's flexible custom loader.
    """
    ...
    args._miles_training_checkpoint_loaded = False
    args._miles_optimizer_scheduler_state_restored = False
    ...
    # miles: a bridge-mode HF or release-checkpoint load is a weights-only
    ...
    if has_local_checkpoint_manager:
        ...
        if training_checkpoint_dir is not None:
            logger.info(
                "Load Miles %s training checkpoint through Megatron (path=%s)",
                expected_role,
                training_checkpoint_dir,
            )
            ...
            args._miles_training_checkpoint_loaded = True
            args._miles_optimizer_scheduler_state_restored = _optimizer_scheduler_state_was_restored(
                args,
                optimizer,
                opt_param_scheduler,
                skip_load_to_model_and_opt=skip_load_to_model_and_opt,
        ...
        if load_training_state and getattr(args, "no_load_optim", False):
            ...
            result = (0, result[1])
        args._miles_training_checkpoint_loaded = _legacy_megatron_load_restored_training_iteration(
            args,
            result,
            load_training_state=load_training_state,
        ...
        if args._miles_training_checkpoint_loaded:
            args._miles_optimizer_scheduler_state_restored = _optimizer_scheduler_state_was_restored(
                args,
                optimizer,
                opt_param_scheduler,
            ...
            if loaded:
                ...
                if iteration is not None:
                    ...
                    if optimizer is not None and opt_param_scheduler is not None:
                        args._miles_optimizer_scheduler_state_restored = True
            else:
                logger.warning(
...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, checkpointing_context, skip_load_to_model_and_opt):
    ...
    # miles path above so its contract (and tests patching it) stay intact.
    if (
...
def save_checkpoint_with_lora(iteration, model, optimizer, opt_param_scheduler):
    """Extended save that handles LoRA adapters separately (miles contract).
