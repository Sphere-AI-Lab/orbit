...
_MODEL_OPT_COMPAT_PATCH_ATTR = "_miles_modelopt_config_compat_patched"
...
def load_hf_config(args):
    ...
    return _load_hf_config(args.hf_checkpoint)
...
def _replace_meta_module_tensor(module: nn.Module, name: str, payload: torch.Tensor, *, is_buffer: bool) -> bool:
    ...
    if (target_tensor is None or target_tensor.device.type != "meta") and hasattr(owner, "to_wrap"):
        ...
        wrapped_tensor = (
            wrapped_owner._buffers.get(leaf_name) if is_buffer else wrapped_owner._parameters.get(leaf_name)
        )
...
_NVFP4_EXPERT_PACKED_WEIGHT_RE = re.compile(r"^(?P<module>.*\.experts\.linear_fc[12])\.weight(?P<idx>\d+)(?:_[wv])?$")
...
def _dist_checkpoint_already_loaded(parallel_model, dist_weight_path, prefix="") -> bool:
    ...
    for model in parallel_model:
        if getattr(model, "_miles_loaded_dist_checkpoint_path", None) != expected_path:
            return False
        if getattr(model, "_miles_loaded_dist_checkpoint_prefix", None) != expected_prefix:
            return False
    return True
...
def _mark_dist_checkpoint_as_loaded(parallel_model, dist_weight_path, prefix="") -> None:
    ...
    for model in parallel_model:
        model._miles_loaded_dist_checkpoint_path = loaded_path
        model._miles_loaded_dist_checkpoint_prefix = loaded_prefix
...
def _modelopt_state_already_restored(parallel_model, checkpoint_path) -> bool:
    ...
    return bool(parallel_model) and all(
        getattr(model, "_miles_restored_modelopt_checkpoint_path", None) == expected_path for model in parallel_model
    )
...
def _mark_modelopt_state_restored(parallel_model, checkpoint_path) -> None:
    ...
    for model in parallel_model:
        model._miles_restored_modelopt_checkpoint_path = restored_path
...
def _restore_modelopt_state_before_load(parallel_model, checkpoint_path) -> bool:
    ...
    # MILES_RESTORE_MODELOPT_STATE=0 to skip the restore. Default remains "1"
    # so existing NVFP4 / future low-precision flows keep their current
    # behaviour.
    if os.environ.get("MILES_RESTORE_MODELOPT_STATE", "1") == "0":
        logger.info(
            "MILES_RESTORE_MODELOPT_STATE=0; skipping ModelOpt state restore for %s",
            checkpoint_path,
        )
        return False
...
def load_dist_checkpoint(parallel_model, dist_weight_path: str, *, is_value_model: bool = False, prefix: str = ""):
    ...
    try:
        for model in unwrapped_parallel_model:
            ...
            validate_access_integrity = torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1
