...
def _patch_bridge_oft_embedding_weight_proxy() -> None:
    ...
    if current is None:
        ...
        logger.warning(
            "OFTVocabParallelEmbedding already exposes non-property weight=%r; " "leaving Bridge behavior unchanged.",
            current,
        )
...
def create_oft_instance(args: Namespace):
    """Return the OFT PEFT instance used by miles.
    ...
    adapter_dtype = (
        ...
        else torch.bfloat16 if getattr(args, "bf16", False) else torch.float32
    )
    ...
    if _oft_type(args) == "oft":
        ...
        logger.warning(
            "OFT: legacy shared-R (--oft-type oft) — fused QKV/gate-up share one rotation. " "targets=%s exclude=%s",
            target_modules,
            exclude_modules,
        )
        return OFT(
