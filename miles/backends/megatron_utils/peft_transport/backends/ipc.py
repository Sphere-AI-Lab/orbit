...
from miles.backends.megatron_utils.sglang import MultiprocessingSerializer
from miles.backends.megatron_utils.update_weight.sync_metrics import get_payload_tracker
...
def _cpu_gather_transport_enabled() -> bool:
    ...
    Set MILES_PEFT_ADAPTER_TRANSPORT=cpu_gather on runtimes where trainer-side
    ...
    """
    return os.environ.get("MILES_PEFT_ADAPTER_TRANSPORT", "cuda_ipc").strip().lower() == "cpu_gather"
...
class IpcBackend(PeftWeightTransport):
    ...
    def send_adapter(
        ...
        if is_src:
            ...
            cpu_tensors = {
                name: tensor.detach().to(device="cpu", copy=True).contiguous() for name, tensor in weight_tensors
            }
    ...
    def _synchronize_source_result(
        ...
        results = [result for record in source_records if record is not None for result in record["results"]]
