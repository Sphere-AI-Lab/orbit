...
class RayObjectBackend(PeftWeightTransport):
    ...
    def _send_unshaped_lora_adapter(
        ...
        if self._peft_loaded:
            unload_refs = [
                engine.unload_lora_adapter.remote(lora_name=self.sync_spec.adapter_name) for engine in self._engines
            ]
    ...
    def _update_weight_version(self, weight_version: int) -> tuple[list[ObjectRef], list]:
        version_refs = [
            engine.update_weight_version.remote(weight_version=str(weight_version)) for engine in self._engines
        ]
