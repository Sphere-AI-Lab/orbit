...
from miles.utils.adapter_tensors import AdapterTensorKey, adapter_named_parameters, adapter_tensor_key_digest
...
def _validate_no_embedded_distributed_parameter_state(optimizer_state: Any) -> None:
    """Reject parameter state that bypasses Miles's external-state preflight.
...
def _checkpoint_consensus_group():
    ...
    if str(dist.get_backend()).lower().endswith("gloo"):
        return None
    raise RuntimeError("PEFT checkpoint consensus requires Miles's world Gloo process group")
...
def preflight_peft_adapter_checkpoint(adapter_path: str | Path) -> PeftCheckpointPreflight:
    ...
    if inconsistencies:
        raise RuntimeError(
            "PEFT checkpoint preflight found inconsistent rank-local files: " + "; ".join(inconsistencies)
        )
...
try:
    ...
    HAVE_BRIDGE_ORBIT = True
except ImportError:
    # Pre-orbit Megatron-Bridge (the bridge-orbit port has not landed in this
    # env). OFT itself is unusable without it — create_peft_instance raises on
    # first use — but LoRA and non-PEFT paths must keep importing fine.
    HAVE_BRIDGE_ORBIT = False
    CANONICAL_OFT_SLICE_NAMES: tuple[str, ...] = ()
...
def save_training_state(
    ...
    *,
    ...
    _coordinated_checkpoint_call(
        ...
        lambda: validate_distributed_optimizer_sources_for_save(optimizer) if has_external_parameter_state else None,
    )
    _coordinated_checkpoint_call(
    ...
    for gbuf_idx, gbuf_range_maps in enumerate(gbuf_ranges):
        ...
        for bucket_idx, (range_map, bucket) in enumerate(zip(range_maps, buckets, strict=True)):
            ...
            if padded_numel <= 0 or padded_numel % gloo_size != 0:
                raise RuntimeError(f"distributed optimizer gbuf {gbuf_idx} bucket {bucket_idx} padded size is invalid")
            ...
            if not isinstance(param_map, Mapping):
                raise RuntimeError(f"distributed optimizer gbuf {gbuf_idx} bucket {bucket_idx} has no parameter map")
    ...
    for gbuf_idx, gbuf_range_maps in enumerate(optimizer.gbuf_ranges):
        ...
        for key in ("param", "exp_avg", "exp_avg_sq"):
            ...
            if (
                ...
                raise RuntimeError(f"distributed optimizer gbuf {gbuf_idx} checkpoint tensor {key!r} is incompatible")
        for bucket_idx, bucket in enumerate(buffer.buckets):
            padded_numel = int(bucket.grad_data.numel())
            if padded_numel <= 0 or padded_numel % gloo_size != 0:
                raise RuntimeError(f"distributed optimizer gbuf {gbuf_idx} bucket {bucket_idx} is incompatible")
    return normalized
            ...
            for bucket_idx, range_map in enumerate(range_maps):
                for model_param, param_range_map in range_map["param_map"].items():
                    ...
                    if type(index) not in (tuple, list) or len(index) != 2:
                        raise RuntimeError(f"distributed optimizer {context} model-parameter group index is invalid")
                    ...
                    if not 0 <= group_index < len(param_groups):
                        raise RuntimeError(f"distributed optimizer {context} parameter-group index is out of range")
                    ...
                    if not isinstance(group_params, Sequence) or not 0 <= group_order < len(group_params):
                        raise RuntimeError(f"distributed optimizer {context} parameter order is out of range")
...
def _dispatch_external_parameter_state(
    ...
    if plan.is_custom:
        ...
        with tempfile.NamedTemporaryFile(prefix="miles-peft-optimizer-state-", suffix=".pt") as cached_file:
...
def _validate_training_state_payload(
    ...
    *,
    ...
    if expected_iteration is not None and (
        ...
        raise RuntimeError("PEFT checkpoint iteration does not match the expected checkpoint binding")
    ...
    if expected_active_student_version is not None and active_student_version != expected_active_student_version:
        raise RuntimeError("PEFT checkpoint active student version does not match the expected checkpoint binding")
    ...
    *,
    ...
    external_parameter_state_plan = (
        _coordinated_checkpoint_call(
            ...
            lambda: (
                _build_external_parameter_state_plan(
                    optimizer,
                    parameter_state_path,
                    checkpoint_preflight.optimizer_parameter_state_binding,
                )
...
def _restore_prepared_training_state(
    ...
    _coordinated_checkpoint_call(
        ...
        lambda: (
            _validate_external_parameter_state_destinations(external_parameter_state_plan)
...
def restore_peft_training_state_after_optimizer_build(
    ...
    *,
    ...
    if restored_iteration != expected_iteration:
        raise RuntimeError("PEFT optimizer training-state iteration does not match the model/adapter resume iteration")
    return True
