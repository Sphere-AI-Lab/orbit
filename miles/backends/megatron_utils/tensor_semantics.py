...
def _has_triplet_buffers(owner: nn.Module, base_name: str) -> bool:
    return all(
        isinstance(getattr(owner, f"{base_name}_{suffix}", None), torch.Tensor)
...
def _has_nvfp4_expert_buffers(owner: nn.Module, base_name: str) -> bool:
    ...
    has_split_packed_weight = all(
        isinstance(getattr(owner, f"weight{idx}_{suffix}_packed", None), torch.Tensor) for suffix in ("w", "v")
    )
    has_scales = all(
        isinstance(getattr(owner, f"weight_{suffix}{idx}", None), torch.Tensor) for suffix in ("scale", "double_scale")
    )
...
def describe_named_tensor(module: nn.Module, name: str, tensor: torch.Tensor) -> TensorSemantics:
    ...
    for owner in owners:
        ...
        if (leaf_name == "weight" or _NVFP4_EXPERT_WEIGHT_RE.match(leaf_name)) and _has_any_nvfp4_expert_buffers(
            owner
        ):
            is_placeholder = tensor.device.type == "meta" or tensor.numel() == 0
            return TensorSemantics(
