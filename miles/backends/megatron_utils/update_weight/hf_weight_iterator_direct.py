...
from .common import (
    ...
    # ORBIT-SEAM: orbit's expert-TP predicate (DSV4 shared-expert / grouped-MoE-OFT names) replaces
    # upstream's is_routed_expert_param at the two routing sites below
...
def _get_megatron_full_params(
    ...
    if ep_size > 1:
        handles = []
        for info, param in zip(megatron_local_param_infos, params, strict=False):
            # ORBIT-SEAM: EP-aware param routing goes through orbit's predicate, not upstream's
...
def _get_param_full_size(info: ParamInfo) -> int:
    # ORBIT-SEAM: EP-aware TP sizing goes through orbit's predicate, not upstream's
    # is_routed_expert_param
    if uses_expert_tensor_parallel_group(info.name):
