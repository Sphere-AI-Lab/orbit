...
# ORBIT-SEAM: expert-unit routing uses orbit's expert-TP predicate (DSV4 shared-expert /
# grouped-MoE-OFT names) rather than upstream's is_routed_expert_param
from ..common import uses_expert_tensor_parallel_group
...
def _is_expert_update_unit(update_unit: list[tuple[str, torch.Tensor]]) -> bool:
    ...
    return uses_expert_tensor_parallel_group(name)
...
class DistBucketedWeightUpdateMixin:
    ...
    def _get_weight_transfer_update_units(self, is_expert: bool) -> list[list[tuple[str, torch.Tensor]]]:
        ...
        for unit in update_units:
            # ORBIT-SEAM: same orbit expert-TP predicate as _is_expert_update_unit
            assert len({uses_expert_tensor_parallel_group(name) for name, _tensor in unit}) == 1, [
                name for name, _tensor in unit
            ]
    ...
    def update_weights(self) -> None:
        ...
        # ORBIT-SEAM: SUM per-rank contributions (collective on the gloo group, reached by
