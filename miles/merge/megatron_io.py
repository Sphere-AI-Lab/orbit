"""Merge miles Megatron-native OFT adapter shards (adapter_megatron_tp{tp}_pp{pp}.pt).
...
from miles.merge import get_strategy
from miles.merge.strategy import StateDict
...
def merge_megatron_adapters(
    ...
    for shard in shards_per_adapter[0]:
        state_dicts = [torch.load(Path(d) / shard, map_location="cpu", weights_only=True) for d in adapter_dirs]
