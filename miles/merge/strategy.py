...
StateKey: TypeAlias = str | tuple[int, str]
StateDict: TypeAlias = dict[StateKey, torch.Tensor]
...
def get_strategy(name: str) -> MergeStrategy:
    if name not in _REGISTRY:
        raise KeyError(f"unknown merge method {name!r}; available: {sorted(_REGISTRY)}")
