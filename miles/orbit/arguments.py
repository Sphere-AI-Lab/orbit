...
``miles.orbit.*`` imports below are function-local for the same reason the originals were:
...
def add_on_policy_distillation_arguments(parser):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "http://host:port) used by miles.orbit.rewards.llm_judge.reward_func. "
...
def _validate_judge_args(args) -> None:
    ...
    if not getattr(args, "judge_base_url", None):
        raise ValueError(
            "--custom-rm-path miles.orbit.rewards.llm_judge.reward_func requires --judge-base-url "
...
def _validate_reward_router_args(args) -> None:
    ...
    if not getattr(args, "group_rm", False):
        raise ValueError(
            "--custom-rm-path miles.orbit.rewards.reward_router.reward_func is a batch-mode hook: "
...
def _validate_genrm_args(args) -> None:
    ...
    if not getattr(args, "group_rm", False):
        raise ValueError(
            "--custom-rm-path miles.orbit.rewards.genrm_judge.reward_func is a batch-mode hook: "
            "it must be combined with --group-rm (otherwise it would receive single samples)."
        )
    if not getattr(args, "judge_base_url", None):
        raise ValueError(
            "--custom-rm-path miles.orbit.rewards.genrm_judge.reward_func requires --judge-base-url "
            "<http://judge-host:port> (an OpenAI-compatible chat-completions server)."
        )
...
def validate_opd_topk_loss_args(args) -> None:
    ...
    # teacher URL, miles.orbit.opd.opd_scoring.opd_score_sample via local_scoring_enabled)
    ...
    from miles.orbit.opd.opd_scoring import local_scoring_enabled
    ...
    if local_scoring_enabled(args):
        raise ValueError(
            ...
            "with no external teacher URL) scores through miles.orbit.opd.opd_scoring.opd_score_sample, "
            "which does not retain teacher_topk_ids/teacher_topk_logprobs -- that transport lives "
            "only on the external-URL scoring path (miles.orbit.opd.opd_sglang.post_process) in v1."
        )
    ...
    expected_rm = "miles.orbit.opd.opd_sglang.reward_func"
    expected_post = "miles.orbit.opd.opd_sglang.post_process"
    if (
...
def _validate_opd_args(args) -> None:
    ...
    from miles.orbit.opd.opd_teacher_spec import is_same_base, is_self_teacher, parse_teacher_spec
        ...
        if external:
            ...
            if spec.source == "adapter":
                if not os.path.isdir(spec.path):
                    raise FileNotFoundError(f"--opd-teacher adapter: {spec.path} does not exist.")
                _validate_teacher_adapter_config(spec.path, args.peft_method)
            if getattr(args, "custom_rm_path", None) == "miles.orbit.opd.opd_sglang.reward_func":
                raise ValueError(
                    "Local-teacher mode scores through the built-in rollout stage; do not set "
                    "--custom-rm-path miles.orbit.opd.opd_sglang.reward_func (it would double-score). "
...
def _is_peft_enabled(args) -> bool:
    ...
    Avoids importing miles.orbit.megatron.peft_utils here (that
...
def _assert_critic_supports_bridge(args) -> None:
    ...
    # `use_critic` is derived in `_apply_critic_args`, which runs after the
    # `_apply_bridge_load_path` call site; mirror the derivation there so this
    # pure argument check still fires before any checkpoint I/O.
    separate_critic = (
        uses_separate_critic(args)
        if hasattr(args, "use_critic")
        else args.advantage_estimator == "ppo"
        and getattr(args, "critic_mode", "full") == "full"
    )
    if separate_critic:
...
def _apply_bridge_load_path(args) -> None:
    ...
    from miles.orbit.megatron.low_precision_bootstrap import (
...
def orbit_normalize_peft_args(args) -> None:
    ...
    from miles.orbit.true_on_policy import apply_true_on_policy_parse_defaults
...
def add_orbit_arguments(parser):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(miles/orbit/true_on_policy/). The current Phase 1-4 implementation aligns "
