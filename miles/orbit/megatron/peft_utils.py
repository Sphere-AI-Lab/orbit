...
from miles.orbit.utils.adapter_tensors import AdapterTensorKey, adapter_named_parameters, adapter_tensor_key_digest
...
def create_peft_instance(args):
    ...
    if method == "oft":
        from miles.orbit.megatron.oft_utils import create_oft_instance
...
def build_peft_sync_spec(args) -> PeftSyncSpec | None:
    method = get_peft_method(args)
    if method == "oft":
        from miles.orbit.megatron.oft_utils import OFT_ADAPTER_NAME, build_oft_sync_config
...
def save_peft_checkpoint(
    ...
    *,
    ...
    if method == "lora":
        ...
        from miles.orbit.megatron.oft_utils import save_oft_checkpoint
    ...
    if self_teacher is not None:
        from miles.orbit.opd.self_teacher_checkpoint import TeacherCheckpointError, save_self_teacher_sidecar
...
def load_peft_adapter(
    ...
    *,
    ...
    if method == "oft":
        from miles.orbit.megatron.oft_utils import load_oft_adapter
