...
(``miles/ray/rollout.py::_compute_zero_std_metrics``, called from
...
(``miles/utils/arguments.py``). So ``reward_func`` stashes the raw teacher
...
from miles.orbit.rewards.scoring_client import post_json
...
def _full_vocab_payload(input_ids: list[int]) -> dict[str, Any]:
    ...
    (miles/orbit/opd/teacher_lm_head.py). The teacher server must
