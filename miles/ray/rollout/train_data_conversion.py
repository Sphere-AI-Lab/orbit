...
# ORBIT-SEAM (re-anchored from miles/ray/rollout.py::_convert_samples_to_train_data, which upstream
# dbbab156 split into this module): collect_teacher_topk_data assembles the direct-OPD top-k pair
...
def convert_samples_to_train_data(
    ...
    # ORBIT-SEAM: `any(...)` + a missing-count check replaces upstream's samples[0]-only probe -- a
    # teacher producer that scores only part of the batch must fail loudly, not silently ship a
    # short/misaligned teacher_log_probs list
    ...
    # ORBIT-SEAM: same every-sample validation as teacher_log_probs above
    ...
    # ORBIT-SEAM: direct-OPD top-k teacher-logprob train-data assembly. The retained pair is validated
    # together immediately before assembly: every sample must carry rectangular [R, K] rows of the
    # configured K. These keys carry no ROLLOUT_DATA_VALUE_SPEC entry (see the module header map) --
    # the default Ray object store ignores the spec, and the mooncake codecs for ragged 2-D teacher
    # payloads are an open follow-up.
    teacher_topk_data = collect_teacher_topk_data(
        samples,
        expected_top_k=getattr(args, "opd_log_prob_top_k", None),
    )
    if teacher_topk_data is not None:
        train_data.update(teacher_topk_data)

    # ORBIT-SEAM: full-vocab OPD teacher hidden states, with the same every-sample validation
...
# numerical-equivalence campaign before treating the orbit module as dead.
