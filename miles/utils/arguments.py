...
from orbit.arguments import (
    ...
    _apply_critic_args,
    _apply_custom_config_args,
    _apply_nested_rope_theta,
    _apply_training_mode_args,
    _finalize_train_offload_args,
    _is_peft_enabled,
    _normalize_peft_args,  # noqa: F401
    _validate_eval_nll_args,
    _validate_judge_args,  # noqa: F401
    _validate_opd_args,  # noqa: F401
    _validate_peft_ref_args,
    _validate_ppo_args,
    add_on_policy_distillation_arguments,  # noqa: F401
    add_orbit_arguments,
    needs_opd_teacher,  # noqa: F401
    orbit_normalize_peft_args,
    orbit_validate_args,
    uses_adapter_critic,  # noqa: F401
    uses_head_critic,  # noqa: F401
    uses_one_trunk_critic,  # noqa: F401
    uses_rollout_engines,
    uses_separate_critic,  # noqa: F401
    validate_async_off_policy_correction,  # noqa: F401
    validate_opd_topk_loss_args,  # noqa: F401
    validate_opd_topk_reference_kl_args,
    validate_opd_topk_vocab_size,  # noqa: F401
    validate_rollout_temperature,
)
...
# ORBIT-SEAM: orbit renamed this provider; the orbit name stays as an alias so existing call sites (tools/dump_args_surface.py, tests) keep resolving
get_orbit_extra_args_provider = get_miles_extra_args_provider
...
def parse_args(add_custom_arguments=None):
    ...
    if backend == "megatron":
        ...
        # ORBIT-SEAM: base's experimental FSDP branch deleted -- orbit does not vendor miles.backends.experimental.fsdp_utils and narrows --train-backend to choices=["megatron"], so this branch is unreachable
...
def miles_validate_args(args):
    # ORBIT-SEAM: orbit runs its --custom-config-path overlay and its training-mode (rl/sft) defaults before the base body, and keeps that body under the module-level name _common_orbit_validate_args so callers can substitute it (tests/test_sft_mode.py monkeypatches exactly this attribute)
...
def _common_orbit_validate_args(args):
    ...
    # ORBIT-SEAM: under PEFT the reference policy is the frozen base, so orbit rejects ref_load/ref_update_interval outright and skips the base full-FT ref-checkpoint validation below
    _validate_peft_ref_args(args)
    ...
    # ORBIT-SEAM: orbit's forward-only held-out NLL eval (--eval-nll-data/-interval/-micro-batch-size) has its own consistency checks
    _validate_eval_nll_args(args)
    ...
    # ORBIT-SEAM: base's LoRA-only target-module parsing replaced by orbit's PEFT superset (--peft-method lora/oft, 'all'/'all-linear' expansion per --peft-variant, adapter paths, DSV4 CP checks) followed by the --true-on-policy expansion
    orbit_normalize_peft_args(args)

    # ORBIT-SEAM: upstream's shared-outer grouped-expert LoRA agreement checks + MoE-expert layout
    # warning below ran inside the `if args.lora_rank > 0:` target-module parsing block that
    # orbit_normalize_peft_args replaces; re-anchored onto orbit's normalized args.target_modules
    if args.lora_rank > 0:
    ...
    # ORBIT-SEAM: orbit's reward-side validator bundle (OPD, LLM judge, generative RM, reward router)
    ...
    # ORBIT-SEAM: base's critic-worker defaulting replaced by orbit's --critic-mode-aware version (full / adapter / head one-trunk topologies)
    ...
    # ORBIT-SEAM: colocate offload defaults only apply when rollout engines are actually used (orbit's SFT mode runs with --no-use-rollout-engines)
    if args.colocate and uses_rollout_engines(args):
        if args.offload_train is None:
            args.offload_train = True
        if args.offload_rollout is None:
            args.offload_rollout = True
        # ORBIT-SEAM: re-anchored onto upstream's --sglang-cuda-graph-backend-prefill (upstream
        # retired sglang_disable_piecewise_cuda_graph / sglang_enforce_piecewise_cuda_graph as
        # ServerArgs). An explicit --sglang-enforce-piecewise-cuda-graph still wins -- orbit leaves
        # the user's prefill backend alone -- instead of being warned about and ignored.
...
def hf_validate_args(args, hf_config):
    ...
    if hasattr(hf_config, "rope_parameters") and isinstance(hf_config.rope_parameters, dict):
        ...
        # ORBIT-SEAM: Gemma-4 nests rope_theta per attention type; orbit falls back to the first nested entry
        else:
            _apply_nested_rope_theta(hf_config)
