...
def convert_checkpoint(
    ...
    # tools/ + miles_plugins/ own conversion, not a vendored miles path); upstream converged on
    # the same repo_base_dir/tools/ entrypoint
...
def execute_train(
    ...
    runtime_env_vars = {
        ...
        "PYTHONUNBUFFERED": "1",
        # ORBIT-SEAM: unconditional now that FSDP is no longer supported (upstream still guards this
        # with `if train_backend_fsdp`, since FSDP's comm overlap could break with it set; orbit
        # rejects --train-backend fsdp outright in execute_train above)
    ...
    # ORBIT-SEAM: base's `cmd_megatron_model_source = (... if megatron_model_type is not None else "")`
    # conditional is gone -- megatron_model_type is now always required (asserted above), so the model
    # args below are unconditional. Upstream converged here by replacing the shell `source` of
    # scripts/models/<type>.sh with shell_safe_model_args().
...
NUM_GPUS_OF_HARDWARE = {
    "H100": 8,
    # ORBIT-SEAM: H200/B200/B300 GPU-per-node entries orbit runs on (upstream converged)
    "H200": 8,
    "B200": 8,
    "B300": 8,
    "GB200": 4,
