"""Runtime construction for the envpack Miles plugin."""
...
from miles_plugins.envpack_adapter.config import EnvpackAdapterConfig, EnvpackConfigError, EnvpackPoolConfig
...
def get_session_client_bundle(config: EnvpackAdapterConfig, args=None):
    ...
    The remote server owns runtime capacity and env lifecycle. The Miles worker
...
def resolve_pool_runtime_demand(args, pool: EnvpackPoolConfig) -> dict:
    ...
    The adapter reports Miles' desired active episode throughput. Envpack is
    ...
    capacity = resolve_miles_active_episode_capacity(args)
    if capacity is None:
...
def resolve_miles_active_episode_capacity(args) -> int | None:
    if args is None:
        return None
