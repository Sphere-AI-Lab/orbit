"""Orbit integration for ``megatron.bridge`` Nemotron-H (Mamba+Attention hybrid).

This plugin is a non-invasive drop-in that:

1. Registers :class:`OrbitNemotronHBridge` for ``NemotronHForCausalLM`` (overriding
...
3. Patches :class:`~megatron.core.models.mamba.MambaModel` ``forward`` to
   transparently swallow the ``loss_mask`` kwarg so orbit's generic training
...
# HF name → provider name → cast. Orbit's generic ``model_provider.py`` does not
...
def _build_bridge_subclass():
    """Build the ``OrbitNemotronHBridge`` class lazily so importing this module
    ...
    class OrbitNemotronHBridge(NemotronHBridge):
        ...
        def provider_bridge(self, hf_pretrained):
            # Install the megatron.core shims lazily — only runs that actually
            # build a Nemotron-H model get the TransformerLayer/MambaModel
            # monkeypatches (miles bbea85561: eager install perturbed unrelated
            # models' tests).
            ...
            # Mirror to moe_layer_freq so orbit's replay_utils registers the
            ...
            # the base CONFIG_MAPPING translates to provider.mtp_num_layers=1. Orbit's
            ...
            # requested via the ORBIT_NEMOTRONH_KEEP_MTP env var.
            import os

            if not os.environ.get("ORBIT_NEMOTRONH_KEEP_MTP"):
...
def _install_nemotronh_hybrid_layer_shims() -> None:
    ...
    if getattr(TransformerLayer, "_orbit_nemotron_hybrid_shim_installed", False):
        return
    ...
    TransformerLayer._orbit_nemotron_hybrid_shim_installed = True


def _install_mamba_model_loss_mask_shim() -> None:
    """Make ``MambaModel.forward`` silently accept (and drop) ``loss_mask``.

    Orbit's generic training loop was written against ``GPTModel.forward``,
    ...
    if getattr(MambaModel, "_orbit_loss_mask_shim_installed", False):
        return
    ...
    MambaModel._orbit_loss_mask_shim_installed = True


def install() -> None:
    """Register the Nemotron-H bridge subclass.

    The megatron.core shims are installed lazily from ``provider_bridge`` so
    they only apply to runs that actually build a Nemotron-H model.
    """
    for fn in (_build_bridge_subclass,):
        try:
            fn()
        except Exception as e:  # best-effort; avoid breaking unrelated models
            logger.warning("orbit nemotron_h shim %s not applied: %s", fn.__name__, e)
