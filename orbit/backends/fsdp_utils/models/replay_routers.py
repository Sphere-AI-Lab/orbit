...
def _qwen3_router_forward(self, hidden_states):
    ...
    router_top_value, router_indices = self._orbit_replay_topk(router_probs, self.top_k)
...
def install_qwen3_router_replay(router: nn.Module) -> None:
    ...
    router._orbit_replay_topk = routing_replay_manager.get_topk_fn(
...
def _glm4_moe_lite_route_tokens_to_experts(self, router_logits):
    ...
    topk_indices = self._orbit_replay_topk(scores_for_choice, self.top_k)
...
def install_glm4_moe_lite_router_replay(block: nn.Module) -> None:
    ...
    block._orbit_replay_topk = routing_replay_manager.get_topk_fn(
