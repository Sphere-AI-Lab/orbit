...
from orbit.backends.megatron_utils.rematerialize_utils import build_main_cast_context
from orbit.dashboard import hooks as dashboard_hooks
from orbit.ray.train_actor import TrainRayActor
from orbit.utils import train_dump_utils
from orbit.utils.argparse_utils import inplace_modify_args
from orbit.utils.arguments import resolve_train_offload_mode
from orbit.utils.audit_utils.event_logger.logger import event_logger_context
from orbit.utils.audit_utils.witness.allocator import WitnessInfo
...
from orbit.utils.multi_lora import is_multi_lora_enabled
...
from orbit.utils.replay_base import all_replay_managers, routing_replay_manager
from orbit.utils.test_utils.ft_test_actions import FTTestActionActorExecutor
from orbit.utils.timer import Timer, inverse_timer, timer
from orbit.utils.tracking_utils.structured_log import with_logs
...
if TYPE_CHECKING:
    from orbit.ray.rollout.rollout_manager import EnginesAndLock
...
class MegatronTrainRayActor(TrainRayActor):
    def _uses_flat_train_offload(self) -> bool:
        # The Orbit path is PEFT-specific. Shared-PPO critics remain on Orbit's
        # full-model TMS backend even when the actor opts into flat offload.
        return getattr(self, "role", "actor") == "actor" and resolve_train_offload_mode(self.args) == "flat"
    ...
    def init(
        ...
        *,
        ...
        self._asleep = False
        self._wake_up_stream = (
            torch.cuda.Stream()
            if self.args.offload_train
            and self._uses_flat_train_offload()
            and getattr(self.args, "offload_train_async", False)
            else None
        )
        ...
        if self.args.colocate or get_peft_method(self.args) != "none":
            ...
            if self.args.update_weight_transfer_mode == "broadcast":
                ...
                # Lazy import: keep Mooncake's transfer engine off non-P2P actor paths.
    ...
    def sleep(self) -> None:
        ...
        if self._uses_flat_train_offload():
        ...
        elif self.args.rematerialize_param_from_master_weight and self.role == "actor":
    ...
    def prefetch_train_state(self, rollout_id: int) -> None:
        """Start the flat backend's H2D copies while rollout work is winding down."""
        if not self.args.offload_train or not self._asleep or not self._uses_flat_train_offload():
            return
        if rollout_id < getattr(self.args, "num_critic_only_steps", 0):
            return
        if not getattr(self.args, "offload_train_async", False) or self._wake_up_stream is None:
            return
    ...
    def wake_up(self) -> None:
        ...
        if self._uses_flat_train_offload():
            if self._wake_up_event is not None:
                torch.cuda.current_stream().wait_event(self._wake_up_event)
                self._wake_up_event = None
    ...
    def compute_ref_log_probs(
        ...
        """Compute reference log-probs when the configured loss needs them."""
    ...
    def train_actor(
        ...
        *,
        ...
        with inverse_timer("train_wait"), timer("train"):
            ref_data = self.compute_ref_log_probs(data_iterator, num_microbatches, rollout_id)
        ...
        if train_step_outcome == TrainStepOutcome.NORMAL and is_multi_lora_enabled(self.args):
            from orbit.backends.megatron_utils.multi_lora_utils import commit_trained_batch
    ...
    def reconcile_adapters(self) -> None:
        ...
        if not is_multi_lora_enabled(self.args):
            return
        from orbit.backends.megatron_utils.multi_lora_utils import cleanup_adapters as _cleanup_adapters
        from orbit.backends.megatron_utils.multi_lora_utils import load_adapters as _load_adapters
        from orbit.ray.multi_lora.controller import get_multi_lora_controller
    ...
    def save_model(self, rollout_id: int, force_sync: bool = False) -> None:
        ...
        if is_multi_lora_enabled(self.args):
            from orbit.backends.megatron_utils.multi_lora_utils import save_due_adapter_checkpoints
        ...
        if self.args.save_hf is not None and self.role == "actor":
            from orbit.backends.megatron_utils.hf_export import save_hf_model
    ...
    def update_weights(self, info: "EnginesAndLock") -> None:
        ...
        if is_multi_lora_enabled(self.args):
            from orbit.backends.megatron_utils.multi_lora_utils import select_adapters_to_push
        ...
        with torch_memory_saver.disable() if self.args.offload_train else nullcontext():
            ...
            if is_multi_lora_enabled(self.args):
                from orbit.backends.megatron_utils.multi_lora_utils import commit_weight_push
            ...
            if self._uses_flat_train_offload() and self.args.offload_train_adapter:
