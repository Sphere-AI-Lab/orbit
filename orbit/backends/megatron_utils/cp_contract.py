"""Small helpers for keeping Orbit and Megatron CP settings in sync."""
...
def canonicalize_cp_comm_type(value, *, default_to_p2p: bool = True) -> str | None:
    ...
    Orbit currently has one data/loss layout decision for the whole model, so a
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
        ...
        if len(unique_values) != 1:
            raise ValueError("Mixed per-layer cp_comm_type is not supported by Orbit yet: " f"{list(value)!r}")
...
def normalize_cp_contract(args: Namespace, model_type: str | None = None) -> None:
    """Normalize the startup CP contract after all Orbit config overrides."""
    ...
    # Orbit consumers should never need to guess whether they received a list.
