...
from orbit.backends.megatron_utils.ft.indep_dp import allreduce_grads_and_losses_across_replicas
from orbit.backends.megatron_utils.ft.types import TrainStepOutcome
from orbit.backends.megatron_utils.local_weight_checksum import dump_local_weight_checksums
from orbit.utils.audit_utils.witness.allocator import WitnessInfo
from orbit.utils.audit_utils.witness.module import witness_dump_and_clear_stale
...
def setup_model_and_optimizer(
    ...
    # which picks the adapter type internally.
        ...
        if (
            ...
            from orbit_plugins.models.inkling.lora import wrap_model_provider_with_inkling_lora
    ...
    if _is_muon_optimizer(config.optimizer):
        ...
        from orbit.backends.megatron_utils.multi_lora_optimizer import build_multi_lora_optimizer
    ...
    if args.stream_optimizer_state_to_disk:
        from orbit_plugins.optimizers.nvme_stream import setup_optimizer_state_streaming
...
def train_one_step(
    ...
    if multi_lora:
        from orbit.backends.megatron_utils.multi_lora_optimizer import reset_grad_metadata_keep_grads
    ...
    if args.ci_test and args.enable_mtp_training and args.rollout_max_response_len <= 128:
        ...
        from orbit.backends.megatron_utils.ci_utils import check_mtp_only_grad
    ...
    if not disable_optimizer and valid_step:
        if multi_lora:
            from orbit.backends.megatron_utils.multi_lora_utils import step_stepped_adapter_slots
    ...
    for step_id in range(num_steps_per_rollout):
        ...
        if args.enable_mtp_training:
            ...
            if "values" in tracker:
                ...
                if args.ci_test:
                    from orbit.backends.megatron_utils.ci_utils import check_mtp_loss
...
def initialize_model_and_optimizer(
    ...
    if (
        ...
        if (Path(args.lora_adapter_path) / "adapter_model.safetensors").exists():
            from orbit_plugins.models.inkling.lora import load_inkling_lora_adapter
