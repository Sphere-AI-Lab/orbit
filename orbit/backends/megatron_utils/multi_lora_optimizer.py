...
def build_multi_lora_optimizer(
    ...
    each child's param groups are tagged with ``orbit_multi_lora_slot`` and narrowed to this rank's shard."""
        ...
        for slot in range(args.multi_lora_n_adapters):
            ...
            for child in children:
                for group in child.param_groups:
                    group["orbit_multi_lora_slot"] = slot
    ...
    optimizer.orbit_slot_child_indices = slot_child_indices
    logger.info(
...
def _slot_children(optimizer, slot: int):
    return [optimizer.chained_optimizers[i] for i in optimizer.orbit_slot_child_indices[slot]]
