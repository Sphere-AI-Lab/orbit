...
from orbit.utils.adapter_config import AdapterRun
...
def create_multi_lora_instance(args: Namespace):
    ...
    from orbit.backends.megatron_utils.lora_utils import convert_target_modules_to_megatron
...
def zero_optimizer_state_for_adapter(optimizer, model, idx: int) -> None:
    ...
    for chained_optimizer in chained:
        ...
        for group in inner.param_groups:
            if group.get("orbit_multi_lora_slot") == idx and "step" in group:
...
def save_multi_lora_checkpoints(
    ...
    from orbit.backends.megatron_utils.lora_utils import convert_target_modules_to_hf
...
def _deregister_adapter(adapter: AdapterRun, args, model, optimizer) -> None:
    ...
    from orbit.backends.megatron_utils.multi_lora_optimizer import zero_adapter_slot_grads

    from orbit.backends.megatron_utils.multi_lora_scheduler import drop_slot_scheduler
...
def load_adapters(args, model, optimizer, adapters) -> int:
    ...
    from orbit.backends.megatron_utils.initialize import is_first_replica_megatron_main_rank
    from orbit.utils.distributed_utils import get_gloo_group
    ...
    if not adapters:
        return 0
    from orbit.backends.megatron_utils.multi_lora_scheduler import install_slot_scheduler
...
def step_stepped_adapter_slots(args, model, optimizer, rollout_data, rollout_id: int, step_id: int) -> float:
    ...
    from orbit.backends.megatron_utils.multi_lora_optimizer import step_adapter_slots
    from orbit.backends.megatron_utils.multi_lora_scheduler import step_slot_schedulers
