...
``orbit.backends.megatron_utils.*`` import (including the featherweight
...
def install_runtime_hooks() -> None:
    ...
    try:
        import orbit_plugins.megatron_bridge  # noqa: F401
    except Exception as _e:  # best-effort; not every environment uses megatron.bridge
        logger.warning("orbit megatron.bridge plugins failed to load: %s", _e)
