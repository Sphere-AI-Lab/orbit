...
class UpdateWeightFromDistributed(DistBucketedWeightUpdateMixin):
    """
    Update distributed engines via NCCL. Each PP rank: group "orbit-pp_{pp_rank}",
    ...
    def connect_rollout_engines(
        ...
        """
        Create NCCL "orbit-pp_{pp_rank}" if PP source (DP=TP=0). Lock prevents concurrent broadcasts.

        Raw mode creates one NCCL group per PP rank because each PP stage owns
        different params. Bridge mode uses a single NCCL group ``orbit-bridge``
        ...
        if self._is_source:
            if self._bridge_mode:
                self._group_name = "orbit-bridge"
            else:
                pp_rank = get_parallel_state().pp.rank
                self._group_name = f"orbit-pp_{pp_rank}"
