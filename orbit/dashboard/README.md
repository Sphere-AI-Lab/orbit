...
Training-dynamics and efficiency dashboards for orbit runs, built on top of
...
`--use-orbit-dashboard` adds a named collector actor on the driver node, one
...
`--dump-details` alone (no `--use-orbit-dashboard`) still writes the rollout
...
python -m orbit.dashboard.serve --dump-details /path/to/dump [--follow] [--port 7788]
```
...
(auto-enabled above 64 lanes). Runs recorded *without* `--use-orbit-dashboard`
...
```bash
python -m orbit.dashboard.serve --demo    # generated demo data, no cluster needed
python -m pytest tests/fast/dashboard/ -q
ORBIT_DASHBOARD_REALDATA_DIR=/path/to/real/dump python -m pytest tests/fast/dashboard/ -q
```
