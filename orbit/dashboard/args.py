"""CLI arguments and configuration plumbing for the orbit dashboard."""

from __future__ import annotations

import logging

from orbit.dashboard.collector import CollectorConfig
...
def add_dashboard_arguments(parser) -> None:
    group = parser.add_argument_group("orbit dashboard")
    group.add_argument(
        "--use-orbit-dashboard",
        action="store_true",
        default=False,
        help="Collect dashboard telemetry (phases, GPU util, engine metrics) under {dump-details}/dashboard/. "
        "Requires --dump-details. View with `python -m orbit.dashboard.serve`.",
    )
...
def validate_dashboard_args(args) -> None:
    if not args.use_orbit_dashboard:
        return
    assert args.dump_details is not None, (
        "--use-orbit-dashboard writes telemetry under {dump-details}/dashboard/ and the "
        "trajectory views read the rollout/train dumps, so --dump-details is required"
    )
    if not args.use_rollout_entropy:
        logger.warning(
            "--use-orbit-dashboard without --use-rollout-entropy: per-token entropy "
...
def collector_config_from_args(args, *, start_ts: float) -> CollectorConfig:
    ...
    return CollectorConfig(
        ...
        run_name=args.wandb_group or "orbit-run",
        start_ts=start_ts,
