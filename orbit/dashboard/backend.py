...
``OrbitDashboardBackend`` adapter in ``tracking_utils``:
...
from orbit.dashboard.args import collector_config_from_args
from orbit.dashboard.collector import COLLECTOR_ACTOR_NAME
from orbit.dashboard.logging_utils import RateLimitedWarner
from orbit.dashboard.store import MetricsRecord, Role
...
def init_dashboard(args, *, primary: bool = True, router_addr: str | None = None, **kwargs) -> None:
    ...
    if primary:
        ...
        from orbit.dashboard.collector import DashboardCollector
        ...
        logger.info(
            "orbit dashboard: telemetry -> %s | view live: python -m orbit.dashboard.serve "
...
def _prometheus_factory():
    """Resolves the orbit prometheus collector from inside the dashboard
    collector actor (its module-global handle is per-process)."""
    import ray

    from orbit.utils.tracking_utils.prometheus_utils import _COLLECTOR_ACTOR_NAME
