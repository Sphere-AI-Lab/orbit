"""Standalone dashboard server over a ``--dump-details`` directory.
    ...
    python -m orbit.dashboard.serve --dump-details /path/to/dump_details \\
...
from orbit.dashboard.advisory import DEFAULT_LOW_MFU
...
def make_demo_dir(target: Path) -> Path:
    ...
    Uses the dummy generators from the test suite, so it requires a orbit
    ...
    try:
        ...
        raise SystemExit("--demo needs the dummy generators under tests/; run from a orbit repo checkout") from e
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.demo:
        ...
        dump_dir = make_demo_dir(Path(tempfile.mkdtemp(prefix="orbit_dashboard_demo_")))
    else:
