...
class _FlatEntry:
    ...
    def get(self) -> torch.Tensor:
        if self.grouped_module is not None:
            ...
            getter = getattr(self.grouped_module, "grouped_fp4_expert_tensors")
...
def _move_trainable_adapter_param_buffers(
    ...
    API instead.
    """
    trainable_adapter_ids = {
        id(param) for _, param in _iter_adapter_named_params(root) if param.requires_grad
    }
    ...
    if covered_adapter_ids != trainable_adapter_ids:
        missing_names = sorted(
            names_by_id.get(param_id, "<unnamed>")
            for param_id in trainable_adapter_ids - covered_adapter_ids
        )
        raise RuntimeError(
...
def _allocate_pinned(total_nelem: int, dtype: torch.dtype, chunk_id: int) -> torch.Tensor:
    # Diagnostic knob: ORBIT_PEFT_OFFLOAD_PIN=0 forces pageable host memory to
    ...
    pin = os.environ.get("ORBIT_PEFT_OFFLOAD_PIN", "1") != "0"
    if not pin:
...
def _move_unplanned_root_to_device(root: torch.nn.Module, device: torch.device) -> tuple[int, int, int]:
    ...
    grouped_module_prefixes, moved_grouped_tensors, skipped_grouped_meta = (
        _move_grouped_fp4_expert_modules_to_device(root, device)
    )
