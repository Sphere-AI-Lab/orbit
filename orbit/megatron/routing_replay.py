"""Orbit's R3 (routing replay) wiring, moved out of ``megatron_utils/replay_utils.py``.

Everything here was added by orbit to a vendored file that upstream ships with
only two functions in it (``_register_replay_list_moe`` and
``get_register_replay_list_func``, both untouched). There is no upstream
counterpart to any of it, so this is a LIFT: the code moves whole, and the one
vendored call site -- ``megatron_utils/actor.py``, right after
``initialize_model_and_optimizer`` -- imports it from here instead. That is what
lets ``miles/backends/megatron_utils/replay_utils.py`` be byte-pristine again.

Only the module-level ``_PATCHED: set[str] = set()`` did not come across: it was
never read anywhere in the vendored file or outside it, so it was dead and is
dropped rather than moved.

The adapter routes Megatron's per-layer ``TopKRouter`` / ``DeepSeekV4Gate``
replay hook through ``miles.utils.replay_base.routing_replay_manager`` instead
of Megatron's own inference-only ``RouterReplay``. The manager's ``get_topk_fn``
is orbit's (orbit/utils/replay_ext.py installs it on the vendored class), so the
replayed expert ids get sanitised on the way through.

Nothing is imported at module scope beyond logging: megatron and torch are
pulled in inside the wiring pass, which only ever runs after the model is built.
"""
