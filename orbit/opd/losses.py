...
from orbit.opd.teacher_lm_head import load_teacher_lm_head
...
def _topk_kl_terms(
    ...
    Padded slots (see `orbit.opd.opd_sglang._TOPK_PAD_LOGPROB`) carry a teacher
...
def _resolve_opd_topk_kl_type(args: Namespace) -> tuple[str, float]:
    """Local counterpart to `orbit.opd.opd_sglang._get_kl_type` -- kept independent
...
def opd_topk_loss_function(
    ...
    # (orbit.opd.opd_sglang._TOPK_PAD_TOKEN_ID/_TOPK_PAD_LOGPROB): the -1e4 underflows
