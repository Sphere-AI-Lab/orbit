...
from orbit.utils.iter_utils import group_by
        ...
        for i in range(len(self.all_engines)):
            ...
            env_vars = build_noset_visible_devices_env_vars() | {
                ...
                for key, default_val in {
                    ...
                    "SGLANG_OFT_MLA_ROTATE_KV_CACHE": "0",
                    "SGLANG_OFT_SINGLE_ADAPTER_BLOCK_S": "64",
                }.items()
            }
        ...
        if (
            ...
            if all(group_index is not None for group_index in group_indices):
                rewards = normalize_grouped_rewards(
                    ...
                    group_indices,
                    normalize_by_std=(
                        self.args.advantage_estimator in ["grpo", "gspo"] and self.args.grpo_std_normalization
                    ),
                )
                return raw_rewards, rewards

            # Fallback for callers that do not populate group_index.
...
def _start_router(args, *, has_pd_disaggregation: bool = False, force_new: bool = False) -> tuple[str, int]:
    """Start sgl router or orbit router and return (router_ip, router_port).
    ...
    if args.use_orbit_router:
        ...
        assert not has_pd_disaggregation, "orbit router does not support PD disaggregation."
        from orbit.router.router import run_router
...
def _start_session_server(args):
    ...
    as a standalone process regardless of whether ``--use-orbit-router`` is active.
    """
    ...
    from orbit.rollout.session.session_server import run_session_server
