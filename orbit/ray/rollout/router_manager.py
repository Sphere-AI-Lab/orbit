...
from orbit.router.router import run_router as run_orbit_router
from orbit.utils.http_utils import _wrap_ipv6, find_available_port, get_host_info, is_port_available
from orbit.utils.http_utils import run_router as run_sglang_router
from orbit.utils.http_utils import wait_for_server_ready
...
def start_router(args, *, has_pd_disaggregation: bool = False, force_new: bool = False) -> tuple[str, int]:
    ...
    if _requires_orbit_router_passthrough(args):
        ...
        logger.warning(
            "Forcing Orbit router for OFT rollout because the installed sglang_router does not preserve "
            "the adapter path."
        )
    ...
    # which transitively loads torch+sglang (minutes on the bare-metal orbit env,
...
def start_session_server(args):
    ...
    Always started standalone regardless of whether ``--use-orbit-router`` is
    active.
    """
    ...
    for port, process in processes:
        wait_for_server_ready(ip, port, process, timeout=300)
