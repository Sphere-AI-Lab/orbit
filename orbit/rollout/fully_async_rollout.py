...
rollout API (the default; incompatible with ``ORBIT_USE_LEGACY_ROLLOUT_V1=1``).

Evaluation targets whatever ``GenerateState`` ``RolloutManager`` passes via
``RolloutFnEvalInput.generate_state`` (see ``orbit/rollout/checkpoint_eval.py``
...
from orbit.rollout.fully_async_data_buffer import (
    DataBuffer,
