...
from orbit.rollout.generate_utils.generate_endpoint_utils import compute_routing_headers, policy_uses_routing_key
from orbit.utils.http_utils import post
from orbit.utils.lora import LORA_ADAPTER_NAME, lora_rollout_enabled
from orbit.utils.multi_lora import slot_lora_name
...
def _lora_path_for_sample(args: Any, sample: Sample) -> str | None:
    """Adapter name to score under: the sample slot's __orbit_slot_{N} name, the fixed single-LoRA name, or None."""
...
def _build_prefill_scoring_payload(
    ...
    if (lora_path := _lora_path_for_sample(args, sample)) is not None:
        ...
        # OFT selects its adapter slot through the supported request key.
