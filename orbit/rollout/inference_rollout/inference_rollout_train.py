...
from orbit.rollout.generate_utils.sample_utils import reward_log_summary, sample_text_preview
...
from orbit.rollout.inference_rollout.inference_rollout_common import GenerateState, generate_and_rm_group
from orbit.rollout.inference_rollout.live_diagnostics import (
    initial_live_log_at,
    maybe_log_all_samples_live_diagnostics,
)
from orbit.rollout.submission_scheduler import make_submission_scheduler
from orbit.utils import dumper_utils
from orbit.utils.http_utils import get, post, router_worker_base_urls
from orbit.utils.misc import as_completed_async, call_agent_abort_hook, load_function
...
async def abort(state: GenerateState, pendings: set, rollout_id: int) -> list[list[Sample]]:
    ...
    async for group in as_completed_async(pendings):
        ...
        # for partial rollout, collect a consumable flat group into the retry buffer
...
async def generate_rollout_async(
    ...
    aborted_samples = partial_aborted_samples + await abort(state, pendings, rollout_id)
