"""Recompute rollout log-probs via a clean SGLang prefill pass.

Port of miles' ``prefill_logprobs.py`` (true-on-policy invariant 7 — log-prob
provenance): instead of trusting log-probs accumulated across decode steps
(whose values depend on KV-cache state, chunked prefill, and batch
composition), re-score each finished sample with one prefill-only request
(``max_new_tokens=0, return_logprob=True``) after flushing the radix/KV cache.
Provenance is recorded in ``sample.metadata["rollout_log_probs_source"]``.

Independently valuable outside true-on-policy: TIS/ICE-POP/OPD all consume
``rollout_log_probs``, and this removes engine-side variance from that signal.

Orbit deviation from miles: ``--recompute-logprobs-via-prefill`` stands alone
(miles asserts it implies ``--true-on-policy-mode``); the true-on-policy
contract derivation (design doc Phase 3) will require it in mode runs instead.
PEFT requests are attached via ``attach_peft_request_payload`` (covers both
LoRA and OFT) rather than miles' lora-only path.
"""
...
def _attach_peft_request_payload(args: Any, payload: dict[str, Any]) -> dict[str, Any]:
    if (getattr(args, "peft_method", "none") or "none") == "none":
        return payload
    # Imported lazily: generate_endpoint_utils transitively imports deep_ep,
    # which needs a CUDA env — keep this module usable on CPU-only hosts for
    # the (always non-PEFT) unit tests.
    from miles.rollout.generate_utils.generate_endpoint_utils import attach_peft_request_payload

    return attach_peft_request_payload(args, payload)
...
def _build_prefill_scoring_payload(
    ...
    _attach_peft_request_payload(args, payload)
