...
- ``SessionServer`` is a FastAPI app plus one shared httpx client; ``do_proxy`` forwards a request to the inference router (sglang or orbit) — which does the load balancing to worker engines — and returns the raw result, or a 502 JSON error on transport failure.
- Session/TITO logic lives in ``core.SessionCore``; ``setup_session_routes`` (``sessions.py``) wires the HTTP routes to it.
- Standalone (own process, own event loop) so sessions also work with the SGLang Rust Router or any other backend, decoupled from the Orbit Router.
...
from orbit.rollout.session.core import ProxyRequest
...
class SessionServer:
    ...
    requests through the inference router (sglang or orbit)."""

    def __init__(self, args, backend_url: str):
        self.backend_url = backend_url
        self.app = FastAPI()

        timeout = getattr(args, "orbit_router_timeout", 600.0)
...
def run_session_server(args, backend_url: str):
    ...
    setproctitle.setproctitle("orbit-session-server")
