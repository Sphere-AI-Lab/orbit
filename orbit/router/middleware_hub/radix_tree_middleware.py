...
from orbit.utils.mask_utils import get_response_lengths
            ...
            if full_text:
                try:
                    ...
                    if getattr(self.router, "verbose", False):
                        print(f"[orbit-router] Successfully cached trajectory with {len(full_token_ids)} tokens")
                except Exception as e:
                    if getattr(self.router, "verbose", False):
                        print(f"[orbit-router] Warning: Failed to cache trajectory: {e}")
        return response
...
async def postprocess_sample_with_radix_tree(args, sample: Sample, output: dict):
    assert not args.partial_rollout, "Currently partial rollout is not supported when using orbit router"
