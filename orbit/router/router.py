...
def run_router(args):
    """
    Run the Orbit router with the specified configuration.
    """
    # Visible to `pkill -9 orbit`; without this the daemon inherits "python".
    setproctitle.setproctitle("orbit-router")

    # Initialize the router with tokenizer and lazy worker initialization
    orbit_router = OrbitRouter(args, verbose=False)

    # Start the server
    uvicorn.run(orbit_router.app, host=args.sglang_router_ip, port=args.sglang_router_port, log_level="info")


class OrbitRouter:
    def __init__(self, args, verbose=False):
        """Initialize the orbit-router with SGLang router address"""
        self.args = args
        ...
        timeout = getattr(args, "orbit_router_timeout", None)
        ...
        for middleware_path in args.orbit_router_middleware_paths or []:
            if self.verbose:
                print(f"[orbit-router] Loading middleware from: {middleware_path}")
    ...
    async def _check_worker_health(self, url):
        ...
        try:
            ...
            if response.status_code == 200:
                return url, True
            logger.debug(f"[orbit-router] Worker {url} is unhealthy (Status: {response.status_code})")
        except Exception as e:
            logger.debug(f"[orbit-router] Worker {url} health check failed: {e}")
        return url, False

    async def _health_check_loop(self):
        """Background loop to monitor worker health and adjust routing pool."""
        interval = self.args.rollout_health_check_interval
        threshold = self.args.orbit_router_health_check_failure_threshold
        ...
        while True:
            try:
                ...
                for url, is_healthy in results:
                    if not is_healthy:
                        ...
                        if failures >= threshold:
                            logger.warning(
                                f"[orbit-router] Worker {url} failed {threshold} consecutive health checks. Marking as DEAD."
                            )
                ...
                logger.debug(
                    f"[orbit-router] Health check complete. {len(self.worker_request_counts) - len(self.dead_workers)} workers healthy."
                )

            except asyncio.CancelledError:
                logger.warning("[orbit-router] Background health check loop is being cancelled.")
                raise
            except Exception as e:
                logger.error(f"[orbit-router] Unexpected error in health check loop: {e}", exc_info=True)
    ...
    async def add_worker(self, request: Request):
        ...
        if worker_url not in self.worker_request_counts:
            ...
            if self.verbose:
                print(f"[orbit-router] Added new worker: {worker_url}")
