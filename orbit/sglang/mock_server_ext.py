...
Why the WHOLE method moved rather than just orbit's additions: most of it is
orbit's, with upstream's ``/generate``, ``/v1/chat/completions``, ``/health`` and
``/abort_request`` registrations still in it. Those are reproduced verbatim below
and must stay that way; they dispatch back into the vendored
``_handle_generate_like_request`` / ``_compute_generate_response`` /
``_compute_chat_completions_response`` through ``self``.

Orbit's added endpoints mock the PEFT weight/adapter-version protocol the real
engine speaks, which is what lets the adapter-transport tests run without a GPU:
...
the vendored ``__init__``. ``_setup_routes`` is called exactly once, from
``__init__``, so this runs exactly when the vendored assignments used to. They
are per-INSTANCE, deliberately not mixin class attributes:
``staged_adapter_versions`` is mutable, and a shared class-level dict would leak
staged versions between two mock servers in one test process.

MRO note: ``_setup_routes`` must NOT be left in ``MockSGLangServer``'s own body.
A class's ``__dict__`` beats every base, so a retained upstream body would shadow
this one and the PEFT endpoints would silently 404 -- which reads, from a test,
as a transport bug rather than a wiring bug. tools/check_shadow_drift.py is the
gate on that.
