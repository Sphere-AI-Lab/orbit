...
from orbit.dashboard.args import add_dashboard_arguments, validate_dashboard_args
from orbit.rollout.checkpoint_eval import is_checkpoint_eval_fn
from orbit.utils.chat_template_utils.tito_tokenizer import TITOTokenizerType
from orbit.utils.environ import enable_experimental_ft_trainer, use_legacy_rollout_v1
...
from orbit.utils.hf_config import is_dsa, load_hf_config
from orbit.utils.logging_utils import configure_logger_raw
from orbit.utils.lora import is_lora_enabled
from orbit.utils.megatron_args_utils import compute_megatron_world_size_except_dp
from orbit.utils.misc import load_function
from orbit.utils.object_store import ObjectStoreBackend
from orbit.utils.tracking_utils.ci_history import RECORD_DIR_ENV

logger = logging.getLogger(__name__)

_OPD_SGLANG_REWARD_FUNC_PATH = "orbit.rollout.on_policy_distillation.reward_func"
_OPD_SGLANG_REWARD_POST_PROCESS_PATH = "orbit.rollout.on_policy_distillation.post_process_rewards"
...
def resolve_rollout_function_paths(args) -> tuple[str, str]:
    ...
    if use_legacy_rollout_v1():
        standard_path = "orbit.rollout.sglang_rollout.generate_rollout"
    else:
        standard_path = "orbit.rollout.inference_rollout.inference_rollout_common.InferenceRolloutFn"
    rollout_path = args.rollout_function_path or standard_path
    if args.fully_async:
        rollout_path = "orbit.rollout.fully_async_rollout.FullyAsyncRolloutFn"
...
def _resolve_rollout_functions(args) -> None:
    if args.partial_rollout and args.mask_offpolicy_in_partial_rollout and not use_legacy_rollout_v1():
        raise ValueError(
            ...
            "class-based rollout path yet; set ORBIT_USE_LEGACY_ROLLOUT_V1=1"
        )
    if args.fully_async:
        assert (
            not use_legacy_rollout_v1()
        ), "--fully-async needs the class-based rollout API; unset ORBIT_USE_LEGACY_ROLLOUT_V1"
...
def get_orbit_extra_args_provider(add_custom_arguments=None):
    def add_orbit_arguments(parser):
        ...
        def add_cluster_arguments(parser):
            ...
            parser.add_argument(
                ...
                help=(
                    ...
                    "defaults to $SCRATCH/orbit_train_offload_<uid>."
                ),
            )
            parser.add_argument(
        ...
        def add_train_arguments(parser):
            ...
            parser.add_argument(
                "--orbit-dsa-topk-backend",
                type=str,
                choices=["torch", "flashinfer"],
                default="torch",
                help="Top-k backend for Orbit DSA indexer.",
            )
            parser.add_argument(
                "--true-on-policy-mode",
                action="store_true",
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                ...
                help=(
                    ...
                    "(see `orbit.rollout.inference_rollout.inference_rollout_common.InferenceRolloutFn` "
            ...
            parser.add_argument(
                ...
                help=(
                    ...
                    "data buffer (see orbit/rollout/fully_async_data_buffer.py). Constructed with "
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                ...
                "For Orbit-maintained fixed templates, leave this unset and pass "
                "--tito-model so Orbit can auto-resolve the registered template. "
        ...
        def add_debug_arguments(parser):
            ...
            parser.add_argument(
                ...
                "(orbit.utils.test_utils.det_process_group), which folds order-sensitive SUM/AVG "
        ...
        def add_session_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=str,
                default="orbit.rollout.session.v2.picker_hub.drop_retries",
            ...
            parser.add_argument(
                ...
                type=str,
                default="orbit.rollout.session.v2.postprocessor_hub.default_postprocess",
...
def parse_args(add_custom_arguments=None):
    ...
    if backend == "megatron":
        ...
        from orbit.backends.fsdp_utils.arguments import load_fsdp_args

        args = load_fsdp_args(extra_args_provider=add_orbit_arguments)
        # TODO: unify this .rank and .world_size w/ indep_dp logics
    ...
    # On iff the CI harness injected ORBIT_CI_GATE_RECORD_DIR (the same env var
    ...
    if backend == "megatron":
        from orbit.backends.megatron_utils.cp_contract import normalize_cp_contract

        # orbit_validate_args applies --custom-config-path overrides, so resolve
        ...
        from orbit.backends.fsdp_utils.arguments import validate_hybrid_shard_args
...
def _validate_opd_task_reward_args(args) -> None:
    ...
    if getattr(args, "custom_rm_path", None) != _OPD_SGLANG_REWARD_FUNC_PATH:
        raise ValueError(f"--opd-log-task-reward requires --custom-rm-path {_OPD_SGLANG_REWARD_FUNC_PATH}.")
    if getattr(args, "custom_reward_post_process_path", None) != _OPD_SGLANG_REWARD_POST_PROCESS_PATH:
        raise ValueError(
            "--opd-log-task-reward requires --custom-reward-post-process-path "
            f"{_OPD_SGLANG_REWARD_POST_PROCESS_PATH}."
        )
...
def resolve_train_offload_mode(args: argparse.Namespace) -> str:
    """Resolve the train-offload backend while preserving Orbit's TMS default."""
...
def _validate_rematerialize_param_from_master_weight(args):
    ...
    from orbit.backends.megatron_utils.lora_utils import is_lora_enabled
...
def orbit_validate_args(args):
    ...
    if "train" in args.ft_components:
        ...
        if getattr(args, "non_persistent_local_ckpt_dir", None) is None:
            args.non_persistent_local_ckpt_dir = "/tmp/orbit_local_ckpt"
    ...
    # maximal four-role surface is best-effort rather than a Orbit-verified
    ...
    if args.use_opd:
        ...
        if args.opd_log_prob_top_k > 0 and args.opd_top_k_strategy != "only-teacher" and not use_legacy_rollout_v1():
            raise ValueError(
                ...
                "which only the v1 rollout produces; set ORBIT_USE_LEGACY_ROLLOUT_V1=1"
            )
        if args.opd_teacher_urls:
            ...
            from orbit.rollout.on_policy_distillation import parse_teacher_urls
    ...
    from orbit.utils.multi_lora import validate_multi_lora_args
    ...
    if args.use_critic:
        ...
        assert not enable_experimental_ft_trainer(), (
            "Shared Actor/Critic PPO is not supported with ORBIT_EXPERIMENTAL_FT_TRAINER=1: the v2 "
            "fault-tolerant train group cannot route critic values or lifecycle options yet. "
            "Unset ORBIT_EXPERIMENTAL_FT_TRAINER or use a non-PPO advantage estimator."
        )
    ...
    if (args.offload_train_target == "disk" or args.stream_optimizer_state_to_disk) and (
        ...
        args.offload_train_disk_dir = os.path.join(os.environ.get("SCRATCH", "/scratch"), f"orbit_train_offload_{uid}")
    ...
    if args.eval_uses_snapshots:
        assert (
            not use_legacy_rollout_v1()
        ), "Snapshot eval requires the class-based rollout API; unset ORBIT_USE_LEGACY_ROLLOUT_V1"
