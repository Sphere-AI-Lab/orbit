...
from orbit.utils.chat_template_utils import deepseek, template
from orbit.utils.chat_template_utils.message_matcher_hub import assert_messages_append_only_with_allowed_role
from orbit.utils.chat_template_utils.token_seq_comparator import TokenSeqComparator
...
class Kimi26TITOTokenizer(TITOTokenizer):
    ...
    orbit is the primary consumer of this TITO family.
    """
...
class DeepSeekV32TITOTokenizer(TITOTokenizer):
    """DeepSeek V3.2 — orbit' vendored copy of the official ``encoding_dsv32``.

    V3.2 ships no jinja chat_template; prompts render through
    ``templates.encoding_dsv32.encode_messages``, and orbit'
...
class DeepSeekV4TITOTokenizer(TITOTokenizer):
    ...
    Like V3.2, V4 ships no jinja chat_template; orbit' ``apply_chat_template``
