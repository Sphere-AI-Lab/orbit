...
def use_legacy_rollout_v1() -> bool:
    result = bool(int(os.environ.get("ORBIT_USE_LEGACY_ROLLOUT_V1", "0")))

    global _printed_legacy_rollout_v1
    if result and not _printed_legacy_rollout_v1:
        print("ORBIT_USE_LEGACY_ROLLOUT_V1=1 is enabled: using the deprecated v1 rollout path")
...
def enable_experimental_ft_trainer() -> bool:
    raw = os.environ.get("ORBIT_EXPERIMENTAL_FT_TRAINER", "0").lower()
    result = raw in ("1", "true", "on", "yes")

    global _printed_experimental_ft_trainer
    if result and not _printed_experimental_ft_trainer:
        print("ORBIT_EXPERIMENTAL_FT_TRAINER=1 is enabled (experimental feature)")
