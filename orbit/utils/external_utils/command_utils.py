...
from orbit.utils.misc import exec_command, exec_command_all_ray_node
...
def convert_checkpoint(
    ...
    # `tools/` + `orbit_plugins/`, not legacy `tools/`.
    fn(
...
def save_to_temp_file(text: str, ext: str):
    path = Path(f"/tmp/orbit_temp_file_{time.time()}_{random.randrange(0, 10000000)}.{ext}")
