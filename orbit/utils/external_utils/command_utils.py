"""
This file is not for orbit framework itself, but as an optional utility to easily launch orbit jobs and tests.
"""
...
from orbit.utils.external_utils.exec_command import exec_command_cpu, exec_command_gpu, exec_command_multi_node
...
def execute_train(
    ...
    external_ray = get_bool_env_var("ORBIT_SCRIPT_EXTERNAL_RAY")
    ...
    exec_command_cpu(
        ...
        # TODO: do we really need this kill? (or can we instead kill orbit)
        # "pkill -9 python; "
        "pkill -9 orbit; "
        "sleep 3; "
        f"{'' if external_ray else 'pkill -9 ray; '}"
    ...
    if get_bool_env_var("ORBIT_SCRIPT_ENABLE_RAY_SUBMIT", "1"):
...
def get_default_wandb_args(test_file: str, run_name_prefix: str | None = None, run_id: str | None = None):
    ...
    return (
        "--use-wandb "
        f"--wandb-project orbit-{test_name} "
...
def get_env_enable_infinite_run():
    return get_bool_env_var("ORBIT_TEST_ENABLE_INFINITE_RUN", "false")
