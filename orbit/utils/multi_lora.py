...
MultiLoRAHTTPServer) lives in ``orbit/ray/multi_lora/``.
"""
...
def define_new_adapter_metrics(snapshot: dict) -> None:
    ...
    from orbit.utils.tracking_utils.tracking import define_step_key_metric_group
...
def validate_multi_lora_args(args: Any) -> None:
    ...
    surface. Called from ``orbit_validate_args``; a no-op for normal runs."""
    ...
    if args.rollout_function_path is None:
        args.rollout_function_path = "orbit.rollout.multi_lora.async_rollout.generate_rollout_multi_lora"
    if args.data_source_path == "orbit.rollout.data_source.RolloutDataSourceWithBuffer":
        args.data_source_path = "orbit.rollout.multi_lora.data_source.MultiLoRAAsyncDataSource"
    ...
    from orbit.utils.environ import enable_experimental_ft_trainer

    assert not enable_experimental_ft_trainer(), (
        "Multi-LoRA is not supported with ORBIT_EXPERIMENTAL_FT_TRAINER=1: the v2 "
