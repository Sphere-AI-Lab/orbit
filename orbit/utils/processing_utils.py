...
from orbit.utils.hf_config import register_hf_config_aliases
...
def load_tokenizer(name_or_path: str, chat_template_path: str | None = None, **kwargs) -> PreTrainedTokenizerBase:
    ...
    if chat_template_path:
        assert os.path.isfile(chat_template_path), (
            ...
            f"Ensure the path is accessible on this node (e.g. inside the orbit repo or on a shared filesystem)."
        )
...
def load_processor(name_or_path: str, **kwargs):
    ...
    if is_inkling_checkpoint(name_or_path):
        from orbit_plugins.models.inkling.mm_processor import InklingTrainProcessor
...
def process_vision_info(prompt, processor):
    ...
    # TODO: temporary solution, will write image utils for orbit later
