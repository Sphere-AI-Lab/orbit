...
One behavioural difference from the vendored edit, stated because it is not
nothing: upstream caches by ``(name, chat_template_path, kwargs)``, and the
vendored seam wrapped BEFORE that store, so every caller shared one wrapper
object. Wrapping what upstream returns means the cache holds the unwrapped
tokenizer and each call builds a fresh wrapper around the same inner tokenizer.
``maybe_wrap_deepseek_v4_tokenizer`` is idempotent and the wrapper holds no
state, so what callers can observe is unchanged except for wrapper identity --
and reaching into upstream's private cache to preserve that identity is a
tighter coupling than the difference is worth.
...
@patch_function(
    ...
    "load_tokenizer",
    upstream_sha="5bcecf74a0afbf644fb0dfdd1bb78ad4a655ddb6c14127138e4485d2d9a48dcf",
    reason=_REASON,
)
