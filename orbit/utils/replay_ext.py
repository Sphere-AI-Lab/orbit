...
_REASON = (
    "upstream repairs a padded (-1) replay slot with `arange(n) % num_experts`, "
    "which can hand a token the same expert twice or an id outside its own topk "
    "row; Megatron's sparse dispatch map then builds a route the model never "
    "produced. Orbit validates the replayed ids and repairs each padded slot "
    "with an expert that does not collide with the rest of that token's row"
)
...
def get_topk_fn(self, old_topk_fn, return_probs):
    ...
    def _get_replay_result(top_indices, scores, topk, *args, **kwargs):
        ...
        # ORBIT: replaces upstream's inline `top_indices[padding_mask] =
        # arange(...) % num_experts` repair -- see _REASON above.
