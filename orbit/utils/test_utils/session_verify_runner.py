"""Boot a real ``orbit`` rollout pipeline + run the multi-role TITO driver.
...
Both forms run the same ``execute_train(--debug-rollout-only)`` path: full orbit
pipeline (sglang + orbit-router with session support) is launched, ``train`` is
skipped, and the rollout drives ``session_verify_agent.run_agent`` against the
session server.

Args flow through orbit' canonical ``parse_args`` Namespace.
...
SESSION_VERIFY_INVARIANT_ARGS: dict[str, Any] = {
    ...
    "rm_type": "random",
    "custom_generate_function_path": "orbit.utils.test_utils.session_verify_agent.generate",
...
def session_verify_extras(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """``add_custom_arguments`` hook for ``orbit.utils.arguments.parse_args``.
    ...
    these via the canonical orbit flags.
    """
    parser.add_argument(
...
def namespace_to_train_args(ns: argparse.Namespace) -> str:
    ...
    Reads orbit-canonical field names off ``ns``; emits the exact flag set
    ...
    declared, regardless of any drift in orbit' upstream default.
    """
    parts: list[str] = [
...
def run_session_verify(args: argparse.Namespace) -> None:
    """Boot ``orbit`` rollout pipeline and run the multi-role driver.
    ...
    ``args`` MUST be a fully-shaped Namespace carrying orbit-canonical field
    ...
    try:
        U.execute_train(
            ...
            extra_env_vars={
                "ORBIT_TITO_MODEL": args.tito_model,
                "ORBIT_SESSION_VERIFY_METRICS_PATH": metrics_path,
            },
        )
        try:
