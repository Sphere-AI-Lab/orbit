# envpack adapter for Orbit

This plugin lets Orbit sample from envpack environments without moving
environment state into Orbit. It is intentionally a thin sampler-side adapter:

```text
Orbit sampler/trainer:
...
```text
orbit_plugins/envpack_adapter/
  config.py             # envpack yaml parsing and Orbit feature guards
  runtime.py            # LocalEnvpackClient vs RemoteEnvpackClient selection
  data_source.py        # Orbit RolloutDataSource from envpack JSONL/EnvSpec
  generate.py           # custom_generate loop; Orbit calls SGLang, envpack steps env
  renderer.py           # SemanticObservation -> Orbit chat messages/media bytes
...
Repo discovery defaults to `$ORBIT_REPO/thirdparty/envpack`; use
`ENVPACK_REPO` only for an explicit override.

There is no `cli/` package here. Orbit already has its own trace/debug
viewer path, so envpack-specific UI export code was removed.

## Plugin Boundary

The adapter is meant to be a self-contained Orbit plugin. It exposes three
surfaces:
...
Offline dataset builder:
  python -m orbit_plugins.envpack_adapter.build_env_dataset
```
...
`orbit_plugins/envpack_adapter/recipes/common.sh` for the envpack plumbing.
...
capacity. The Orbit adapter sends the `pool_id` plus per-sample `env_config`
...
Normal experiment budgets are grouped in Orbit-style arrays in the training
recipe:
...
set the same `ENVPACK_AUTH_TOKEN` in both the Orbit job and the envpack server
...
same Orbit sample and tries the same prompt/env seed/env config again with a
fresh `episode_id`:
...
The JSONL is not a baked environment trajectory. It is a Orbit-owned prompt
index containing:
...
adapter runtime config generated by the Orbit recipe:
...
```text
Orbit EnvpackDataSource -> sample.metadata["envpack"]
Orbit generate.py       -> RemoteEnvpackClient
...
environment that has simulator dependencies installed, then point Orbit at the
resulting `samples.jsonl`. The training-side Orbit environment only needs the
...
orbit-train env:
  ...
  SGLang client/runtime pieces needed by Orbit
...
Then launch Orbit while pointing the adapter at that server:
...
For dynamic-sampling rollouts, Orbit also calls this hook in a lightweight
...
finish a normal rollout step. Orbit' built-in `rollout/zero_std/*` charts are
...
The unit is the Orbit rollout step (`rollout_id`). DAPO can call
...
"50 refill draws". Within a stage, normal Orbit `--rollout-shuffle` still
...
At launch time `launch_orbit.sbatch` resolves the envpack node IP, sets
`ENVPACK_SERVER_URL=http://<env-node-ip>:18081`, starts the server via `srun`,
waits on `/v1/health`, then starts Ray only on the Orbit nodes. The Orbit recipe
still keeps `--actor-num-nodes 1` and `--rollout-num-gpus 8`; the second node is
not available to Orbit.
...
- The envpack adapter still writes VAGEN-shaped debug metadata because current
  Orbit debug dump tooling expects it. Canonical envpack provenance lives
...
- `partial_rollout`, `group_rm`, external reward models, `rollout_external`,
  and Orbit `RadixTreeMiddleware` are rejected for now instead of silently
  producing incompatible samples.
- Non-envpack Orbit R3 is unaffected. Envpack adapter R3 pass-through is not
