"""Configuration parsing for the envpack Orbit plugin."""
...
def validate_runtime_args(args, config: EnvpackAdapterConfig) -> None:
    ...
    if getattr(args, "rollout_external", False):
        raise EnvpackConfigError("envpack adapter is a custom_generate path, not Orbit rollout_external")
    middleware_paths = getattr(args, "orbit_router_middleware_paths", None) or []
    if getattr(args, "use_orbit_router", False) and any("RadixTreeMiddleware" in path for path in middleware_paths):
        raise EnvpackConfigError(
            "envpack adapter MVP does not support Orbit RadixTreeMiddleware postprocessing; "
