...
This is the Orbit compatibility path:

- Orbit still owns tokenization, processor packing, SGLang calls, logprobs,
...
The structure intentionally mirrors Orbit' existing custom-generate contract;
...
from orbit.rollout.generate_utils.generate_endpoint_utils import compute_routing_headers
...
from orbit_plugins.envpack_adapter.config import (
    EnvpackConfigError,
    load_envpack_config,
    validate_pool_env_config_overrides,
    validate_runtime_args,
)
from orbit_plugins.envpack_adapter.renderer import RenderedObservation, observation_to_chat_message
from orbit_plugins.envpack_adapter.runtime import get_client_bundle
    ...
    *,
    ...
    try:
        ...
        sample.metadata.setdefault("envpack", {}).update(
            {
                ...
                "refill_attempt_index": refill_attempt_index,
                "adapter": "orbit_plugins.envpack_adapter.generate",
                "api": config.api,
        ...
        if created is not None and not finalized:
            try:
                await bundle.client.cancel_episode(episode_id, reason="orbit_adapter_cleanup")
            except Exception as exc:
...
def _seed_legacy_debug_metadata(
    ...
    *,
    ...
    """Populate legacy debug metadata for Orbit' current all-samples dumper."""
...
def _encode_messages(
    *,
    ...
    if renderer is None:
        raise EnvpackConfigError("Orbit state has neither processor nor tokenizer")
    ...
    if images:
        raise EnvpackConfigError("envpack observation contains images but Orbit state.processor is None")
...
def _reject_unsupported_videos(rendered: RenderedObservation) -> None:
    if rendered.videos:
        raise EnvpackConfigError(
            "envpack produced video bytes, but Orbit SGLang generate currently exposes image_data only"
        )
