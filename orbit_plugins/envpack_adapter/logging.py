...
so Orbit' default logging remains unchanged. The train all-samples hook also
returns pre-filter prompt-group diagnostics for Orbit to merge into the normal
...
ALL_SAMPLES_PROCESS_PATH = "orbit_plugins.envpack_adapter.logging.process_all_samples"
...
def process_all_samples(
    ...
    *,
    ...
    Orbit calls ``--custom-rollout-log-function-path`` with the final
    ...
    oversampled set and merged into Orbit' normal rollout log. During long
    DAPO refill loops Orbit may call this hook with ``live=True``; that path
