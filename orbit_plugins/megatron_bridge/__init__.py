"""Orbit plugin package for ``megatron.bridge`` integration.

Importing this package is enough to:

* register orbit' bridge subclasses (e.g.
  :class:`~orbit_plugins.megatron_bridge.nemotron_h.OrbitNemotronHBridge`) via
...
* install general-purpose shims that make ``megatron.bridge`` cooperate with
  orbit infrastructure (e.g. ``ReloadableProcessGroup``).
...
def _install_bridge_pp_group_unwrap() -> None:
    ...
    orbit' :class:`~orbit.utils.reloadable_process_group.ReloadableProcessGroup`.
    ...
    from orbit.utils.reloadable_process_group import ReloadableProcessGroup
...
try:
    from orbit_plugins.models.qwen3_vl import install_qwen3_vl_packed_mrope_patch

    install_qwen3_vl_packed_mrope_patch()
except Exception as _e:  # best-effort; Qwen3-VL may be unavailable in some envs
    logger.warning("orbit Qwen3-VL THD packed mRoPE patch not applied: %s", _e)


# Model-specific bridge subclasses. Each submodule self-installs on import.
# Keep imports here so merely importing ``orbit_plugins.megatron_bridge`` is
# enough to pick up every orbit bridge (mirrors ``orbit_plugins.mbridge``).
try:
    from . import nemotron_h  # noqa: F401
except Exception as _e:  # pragma: no cover - defensive
    logger.warning("orbit nemotron_h plugin failed to load: %s", _e)
