...
3. For Nemotron-H models, patches
   ...
   transparently swallow the ``loss_mask`` kwarg so orbit' generic training
...
# HF name → provider name → cast. Orbit' generic ``model_provider.py`` does not
...
def _build_bridge_subclass():
    ...
    class OrbitNemotronHBridge(NemotronHBridge):
        ...
        def provider_bridge(self, hf_pretrained):
            ...
            # Mirror to moe_layer_freq so orbit' replay_utils registers the
            ...
            # the base CONFIG_MAPPING translates to provider.mtp_num_layers=1. Orbit'
...
def _install_mamba_model_loss_mask_shim() -> None:
    ...
    Orbit' generic training loop was written against ``GPTModel.forward``,
    ...
    def forward(self, *args, loss_mask=None, mtp_kwargs=None, **kwargs):
        # process_mtp_loss expects next-token-shifted labels; orbit passes raw tokens.
