...
GPTModel subclass + provider that orbit' custom-model-provider path loads."""
...
from orbit_plugins.models.inkling.layers import (
    InklingDenseMLP,
    InklingMoELayer,
    InklingRouter,
...
class InklingExtra:
    def __init__(self, t: dict):
        ...
        # Debug knobs: defaults are the production recipe; ORBIT_INKLING_* env vars override.
        self.attn_backend = os.environ.get("ORBIT_INKLING_ATTN_BACKEND", "flex")  # flex | te | fa4
        self.sconv_impl = os.environ.get("ORBIT_INKLING_SCONV_IMPL", "triton")  # triton | torch
        self.sconv_packed = os.environ.get("ORBIT_INKLING_SCONV_PACKED", "0") == "1"
        self.freeze_global_scale = os.environ.get("ORBIT_INKLING_FREEZE_GLOBAL_SCALE", "all")  # all | router | none
...
def get_inkling_spec(args, config, vp_stage=None):
    """--spec entry for the orbit standard provider path."""
    import json
...
def inkling_model_provider(pre_process=True, post_process=True, vp_stage=None, *, mm_towers=False):
    ...
    if mm_towers:
        from orbit_plugins.models.inkling.mm_towers import wire_mm_towers
