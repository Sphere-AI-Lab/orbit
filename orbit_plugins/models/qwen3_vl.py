"""orbit-side fix for Qwen3-VL THD packed mRoPE positions (no Megatron-Bridge edit).
...
_PATCHED = "_orbit_qwen3_vl_thd_mrope_patched"
_DEEPSTACK_VIEWLESS_PATCHED = "_orbit_qwen3_vl_deepstack_viewless_patched"
...
def _patch_deepstack_output_view() -> None:
    ...
    def viewless_deepstack_process(self, *args, **kwargs):
...
def _patch_allgather_vision_embeddings_kwarg() -> None:
    ...
    if orig is None or getattr(orig, "_orbit_kwarg_shim", False):
        return

    class _AllGatherVisionEmbeddingsKwargShim:
        _orbit_kwarg_shim = True
...
def _patch_model_forward_and_rope_index() -> None:
    ...
    # Under CP, orbit pre-shards the THD row (slice_with_cp), but the bridge forward re-shards
    # internally via preprocess_packed_seqs (it expects the FULL input). When orbit already
    # sharded, make that internal call an identity that returns orbit' packed_seq_params (so CP
...
def _patch_preprocess_packed_seqs_identity(model_mod) -> None:
    ...
    if orig is None or getattr(orig, "_orbit_identity_wrapped", False):
        return

    def wrapped(input_ids, attention_mask, *args, **kwargs):
        ctx = getattr(_tls, "cp_local", None)
        if ctx is not None:
            # Already CP-local: skip re-sharding and hand back orbit' full-cu packed_seq_params.
            return input_ids, ctx["psp"]
        return orig(input_ids, attention_mask, *args, **kwargs)

    wrapped._orbit_identity_wrapped = True
...
def _prepare_cp_local_context(parsed):
    """When CP has already pre-sharded this THD row, capture orbit' packed_seq_params so the
...
def _natural_to_zigzag_slice(t, cp_size, cp_rank, dim):
    ...
    Mirrors orbit.backends.training_utils.cp_utils.slice_with_cp / natural_to_zigzag_slice:
...
def _reassemble_full_row(gathered, cu, cp_size):
    ...
    orbit' cu_seqlens * cp) locates each segment. For segment ``i`` of full length ``L`` (a
...
def _build_packed_positions(model, parsed, kwargs, orig_get_rope_index):
    ...
    # CP + THD packing: cu_seqlens_q gives the FULL padded per-segment boundaries (orbit
