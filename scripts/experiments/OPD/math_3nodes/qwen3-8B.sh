#!/bin/bash
#
# OPD/math_3nodes/qwen3-8B — on-policy distillation on dapo-math,
...
#
# The training objective stays pure distillation. Deepscaler scores every
# training response and exposes correctness as rollout/raw_reward, while the
# optimization reward remains exactly 0 and never enters advantages. There is
# no separate eval rollout and no --save (nothing checkpointed).
#
...
RUN_NAME=${WANDB_RUN_NAME:-math-3nodes-rkld${OPD_KL_COEF}-topk${OPD_TOP_K}${DAGGER_RUN_SUFFIX}-task-reward}
