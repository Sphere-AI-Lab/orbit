#!/bin/bash
#
# geo3k-vlm-fp8-multi-turn-fully-async-prefetch2-3node — Qwen3-VL-8B, geo3k multi-turn,
# FULLY-ASYNC, with block-wise FP8 on the LANGUAGE tower.
#
# FP8 variant of geo3k-vlm-multi-turn-fully-async-prefetch2-3node.sh; diff the two
# to see only the precision change. That base file's header is the contract for
# the async / multi-turn / prefetch machinery, which is unchanged here.
#
# THIS CHANGES FIVE AXES AT ONCE relative to the 1-node FP8 VLM smoke that
# qualified this path (job 40435: 2B -> 8B, 1 node -> 3, sync -> fully-async,
# single-turn -> multi-turn, bshd -> THD). A failure will not say which axis
# caused it. Kept deliberately because the goal is the real recipe, not a
# controlled ablation.
#
...
#
# geo3k-vlm-multi-turn-fully-async-prefetch2-3node — Qwen3-VL-8B, geo3k multi-turn,
# fully-async rollout (examples/fully_async), 3 nodes (1 trainer + 2 samplers),
# with a 2-batch background prefetch window.
#
# IDENTICAL to geo3k-vlm-multi-turn-fully-async-3node.sh EXCEPT it adds
# --fully-async-prefetch-batches 2. Same 1-trainer + 2-sampler topology, so it
# isolates the effect of prefetch alone (vs the prefetch=1 base) on the same
# hardware; the 4-node prefetch2 recipe then layers trainer scaling on top.
#
...
#   request window    = 128 * n_samples_per_prompt(8) = 1024 sample requests
#   SGLang semaphore  = sglang_server_concurrency(64) * rollout_num_gpus(16) / per_engine(1) = 1024
# So the prefetch window sits exactly at the semaphore limit (no overshoot warning),
...
HF_MODEL_REPO="Qwen/Qwen3-VL-8B-Instruct-FP8"
# Shared models tree is owned by another user; FP8 downloads need a writable home.
HF_MODEL_CACHE_DIR=${HF_MODEL_CACHE_DIR:-$HOME/hf_cache}
HF_DATASETS=(
...
CKPT_ARGS=(
   # FP8 dir drives the ROLLOUT (and the weight-sync quantizer, via its
   # quantization_config); the trainer initializes from BF16 via --ref-load.
   --hf-checkpoint  "$HF_MODEL_DIR"
   --ref-load       "$HF_BF16_DIR"
...
# Language tower only; vision stays BF16 (see header).
FP8_ARGS=(
