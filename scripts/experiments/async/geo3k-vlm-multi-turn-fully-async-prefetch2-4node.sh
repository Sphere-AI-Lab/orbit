#!/bin/bash
#
# geo3k-vlm-multi-turn-fully-async-prefetch2-4node — Qwen3-VL-8B,
# geo3k multi-turn, fully-async rollout (examples/fully_async), 4 nodes
# (2 trainer + 2 samplers), with a 2-batch background prefetch window.
#
# Topology (32 GPUs across 4 nodes, 8 each):
#   - trainer: 2 nodes, 16 GPUs Megatron (--actor-num-nodes 2 --actor-num-gpus-per-node 8)
...
#   JOB_NAME=geo3k-async-mt-prefetch2-4node-8b TIME=72:00:00 NODES=4 MILES_ENV_NAME=miles \
#   bash scripts/slurm/submit.sh async/geo3k-vlm-multi-turn-fully-async-prefetch2-4node
...
ASYNC_ROLLOUT_ARGS=(
   ...
   # lets the background worker keep up to 2 rollout batches actively generating.
...
# TP=4 over 16 trainer GPUs => DP=4 (four replicas). This keeps the sampler
# side fixed and only scales trainer consumption, so queue depth tells us whether
# trainer throughput was the bottleneck in the 3-node version.
...
FT_ARGS=(
   ...
   # defaults. Run 20862 died CLUSTER_DEAD from a SINGLE 30s false-positive health_generate
   # timeout on a busy/paused-but-alive engine (its /health returned 200 just 3s before the
   # kill), which cascaded through the broadcast collective into the trainer's update_weights().
)

# 2 trainer nodes (16 GPUs) + 2 sampler nodes (16 GPUs). NO --colocate.
LAYOUT_ARGS=(
   --actor-num-nodes        2
