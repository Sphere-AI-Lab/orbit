#!/bin/bash
#
# geo3k-vlm-single-turn-fully-async-3node — Qwen3-VL-8B, geo3k SINGLE-turn,
# fully-async rollout (examples/fully_async), 3 nodes (1 trainer + 2 samplers).
# Single-turn variant of geo3k-vlm-multi-turn-fully-async-3node (drops the custom
# multi-turn generate -> default single-turn generate). qkv-format defaults to thd.
#
...
# Fully-async rollout driver. SINGLE-TURN: no --custom-generate-function-path, so
# generate_and_rm_group falls back to the default miles.rollout.sglang_rollout.generate
# (one SGLang call/sample). This is the ONLY difference vs the multi-turn fully-async
# recipe. The default generate records weight_version via Sample.update_from_meta_info,
# so the staleness filter is active (single-turn never had the multi-turn C1 bug).
ASYNC_ROLLOUT_ARGS=(
   --fully-async
