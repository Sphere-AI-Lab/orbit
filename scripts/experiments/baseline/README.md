...
- **`rollout/rollout_train_kl/{k1,k2,k3}/{mean,min,max}`** (rollout panel):
  ...
  **Not emitted under `--use-rollout-logprobs`** unless `--get-mismatch-metrics`
  is also set: p is the trainer recompute, and that forward is skipped, so there
  is nothing to compare against. That covers the whole OPD family
  (`baseline-…-200step`, `10a`, `10b`, the `12a` arm), and the 200-step baseline
  additionally FATALs on `--get-mismatch-metrics`, so the escape hatch is not
  reachable there. Read these curves on the RL arm; on the OPD arm use the
  train-panel `train/*_rollout_kl` instead.
...
| aborted/stale groups always recycled (hardcoded) | `--async-unused-samples-handler {retry,drop}` — **defaults to `drop`** | **pass `retry` explicitly** — every fully-async recipe with a staleness bound now does (`f3aefa8e` for the two gate recipes, `b9bb20ef` for the remaining six — eight recipe files in all; `12-old-policy-scale-common.sh` also pins it in its `_m12_assert_flag_value` block) |
