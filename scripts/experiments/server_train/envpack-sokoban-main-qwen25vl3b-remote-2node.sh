#!/bin/bash
#
# Envpack Sokoban-main GRPO with one Orbit/SGLang node and one envpack server
# node in the same Slurm job.
...
# The base recipe keeps Orbit actor/rollout layout at one 8-GPU node. The
...
source "$SCRIPT_DIR/envpack-sokoban-main-qwen25vl3b-colocate-1node.sh"
