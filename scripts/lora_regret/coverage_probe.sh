...
#      launched once for PROBE_ROLLOUTS rollouts. A method that cannot start,
...
#   `probe_rollouts` and `analyze` exits 4 on any ledger containing one.
#
...
say "plan"
python -m tools.lora_regret.probe plan | column -t
...
run_probe() {
    local matrix="$1" method="$2" only="$3" gpus="$4" devices="$5"
    local ledger; ledger="$(ledger_for "${matrix}" "${method}")"
    local extra=()
    ...
    echo "[probe] ${matrix}/${method} on GPU(s) ${devices}"
    CUDA_VISIBLE_DEVICES="${devices}" GPUS_PER_NODE="${gpus}" \
        python -m tools.lora_regret.sweep \
            ...
            --results "${ledger}" \
            "${extra[@]}" \
        >"logs/lora_regret/probe-${matrix}-${method}.out" 2>&1 \
        || echo "[probe] ${matrix}/${method} FAILED -- see logs/lora_regret/probe-${matrix}-${method}.out" >&2
}
...
if phase_wanted 1; then
    ...
    device=0
    while IFS=$'\t' read -r matrix method arm only gpus metric full project; do
        run_probe "${matrix}" "${method}" "${only}" 1 "${device}" &
        device=$(( (device + 1) % 8 ))
    ...
    done < <(python -m tools.lora_regret.probe plan --gpus 1)
    wait
fi
...
if phase_wanted 4; then
    ...
    while IFS=$'\t' read -r matrix method arm only gpus metric full project; do
        ...
        run_probe "${matrix}" "${method}" "${only}" 4 "${devices}" &
        slot=$(( (slot + 1) % 2 ))
        if (( slot == 0 )); then wait; fi
    done < <(python -m tools.lora_regret.probe plan --gpus 4)
    wait
fi
...
if phase_wanted 8; then
    ...
    while IFS=$'\t' read -r matrix method arm only gpus metric full project; do
        run_probe "${matrix}" "${method}" "${only}" 8 "0,1,2,3,4,5,6,7"
    done < <(python -m tools.lora_regret.probe plan --gpus 8)
fi

# --- the answer ------------------------------------------------------------
say "report"
python -m tools.lora_regret.probe report --ledger "${PROBE_DIR}/*.jsonl"
echo
echo "Per-run stdout: logs/lora_regret/probe-<task>-<method>.out"
echo "Per-arm launcher logs: logs/lora_regret/<arm>.log"
echo "wandb: one project per task (see runbook section 4.5), group = method."
