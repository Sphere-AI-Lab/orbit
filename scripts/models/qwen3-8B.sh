# scripts/models/qwen3-8B.sh — fork compatibility shim (2026-08-18 sync).
#
# Upstream converted every scripts/models/*.sh into python model_args()
# scripts (see the sibling qwen3-8B.py, consumed via
...
MODEL_ARGS=($(MODEL_ARGS_ROTARY_BASE="${MODEL_ARGS_ROTARY_BASE:-}" python3 -c "from orbit.utils.external_utils.model_args_utils import load_model_args; print(load_model_args('qwen3-8B'))"))
