...
| `miles-sync-2026-07-24/` | The v0.5.15 line (miles-imp PR #41). Combined Miles/SGLang sync with OPD and Envpack conflict preservation, exact multimodal scoring patches, fork CI gates, and matched async/OPD validation. |
| `miles-sync-2026-08-18/` | The v0.5.16 line (miles-imp PR pending). Combined Miles/SGLang/Megatron-Bridge sync — 274 upstream commits. Fully-async ported onto upstream's class-based rollout API (#1716/#1717/#2522) with the fail-closed staleness buffer preserved; session server v2; TE 2.17 + Bridge `7f0fb345`. 32 conflicts resolved per-file — `resolution-notes.md` is the canonical writeup and also records the upstream-R3 vs fork-R3 protocol analysis. Fresh-env install + `verify_env.py` validated (jobs 42842/42862); pre-merge regression gate wired under `scripts/experiments/baseline/`. |
