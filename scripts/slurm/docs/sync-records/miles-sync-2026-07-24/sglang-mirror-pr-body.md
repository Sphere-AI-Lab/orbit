...
branch `sync-upstream-20260723`, pin `9dd80e5f8`).

## Contents (base + 4 local patches + review follow-up)
...
| `9dd80e5f8` | fix exact-suffix E2E CI registration | review follow-up; migrates legacy `suite=` metadata to v0.5.15 `stage=` / `runner_config=` so the test is registry-valid and `/rerun-test`-dispatchable |
...
Patch-shipped runtime unit tests on `38d4bbef5`: **54 passed + 13 subtests**
(scoring suffix ×3 files, pretokenized IDs, io_struct). Final tip `9dd80e5f8`
