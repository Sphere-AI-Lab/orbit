...
Commit shape: upstream SHAs preserved under merge commit `4c93a4b4`; fork adjustments in `2a080f79`; the sync record in `720d1a01`. Five further commits landed before publication (`14d126a8`, `7c4e8040`, `6bb4af5b`, `a91c2a29`, `b4b13326`) and the review-repair rounds below added more after it, so the sync skill's single-commit shape (HARD RULE 4) does **not** hold on this branch. Each follow-up is a focused change carrying its own validation; no history was rewritten, and this body plus `divergence.stat` are kept current so every follow-up is represented.
...
| fully-async port | fail-closed staleness buffer (`examples/fully_async/fail_closed_data_buffer.py`), 9 fork recipes migrated from `--rollout-function-path examples.fully_async...` → `--fully-async` (5 under `scripts/experiments/async/`, 4 under `OPD/multimodal/`; both README trees followed), OPD scoring-session close re-wired into `dispose_rollout_function` |
...
| CI-hygiene collision | upstream #2558 added `test_no_test_file_lives_outside_the_tests_tree`, which the fork's pre-existing `examples/vagen/tests/test_env_dynamics.py` violates. That file defines no pytest tests at all — it is a manual probe with `async def main()` and a `__main__` guard — so it is renamed `env_dynamics_probe.py` (references in both README trees updated) rather than parked in `_KNOWN_ORPHANS`, whose docstring says entries exist to be deleted. Missed pre-publication because the test plan ran `tests/fast`, and this rule lives in `tests/ci/test/`. |
| review repairs (post-publication) | PR-review findings, all verified against the tree before fixing: `--fully-async-prefetch-batches` was inert after the upstream rewrite (nothing read it) so migrated recipes ran at depth 1 — now derives `--async-max-concurrent-samples`, with the prefetch-vs-staleness sanity warning restored; `FailClosedDataBuffer` was never selected by any recipe, leaving upstream's fail-open staleness admission live — now the `--fully-async` default (no-op unless `--max-weight-staleness` is set, explicit `--custom-async-data-buffer-path` still wins); `--opd-topk-per-position` was a silent no-op — now rejected in validation with matching help text, and the orphaned comment removed; `scripts/models/qwen3-30B-A3B.sh` now also forwards `MODEL_ARGS_NUM_LAYERS` (its `.py` reads it); `--fully-async-max-completed-queue-groups` warns that it is superseded by `--async-data-buffer-capacity-factor`, and its help text no longer describes the backpressure it lost (it also pointed at a deleted `examples/fully_async/README.md`); `--async-unused-samples-handler retry` extended from the two gate recipes to all eight recipes that set `--max-weight-staleness`; `dispose()` added to `FullyAsyncRolloutFn` (the class-based port dropped the legacy worker's `stop()`/atexit, so in-flight groups outlived teardown and kept calling the OPD session being closed) and then corrected so it logs rather than re-raises a dead worker's exception — `train_async.py` awaits `rollout_manager.dispose()` on the happy path, not in a `finally`, so raising there reported a fully trained, fully checkpointed run as failed |
...
321 files changed, +45,388 / −341 vs the merged upstream tip `fc04f666` — see [divergence.stat](divergence.stat) (slurm launcher/env tooling, skills, sync records, fork recipes, OPD/multimodal/fully-async fork features, envpack adapter).
