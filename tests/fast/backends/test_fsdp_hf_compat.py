...
from orbit.backends.fsdp_utils.adaptations.weight_bridge import _hf_unfuse_experts_expand, get_param_transform
from orbit.backends.fsdp_utils.update_weight_utils import _iter_sync_named_params
...
def test_nemotron_h_post_load_fixup_gating():
    ...
    from orbit.backends.fsdp_utils.adaptations.post_load_fixups import _FIXUPS
...
from orbit.backends.fsdp_utils.adaptations.post_load_fixups import (
    _FIXUPS,
    apply_post_load_fixups,
)

module_name = "orbit.backends.fsdp_utils.models.nemotron_h"
...
def test_reload_nemotron_h_clobbered_weights_behavior(tmp_path, caplog):
    ...
    from orbit.backends.fsdp_utils.models.nemotron_h import reload_nemotron_h_clobbered_weights
    ...
    with caplog.at_level(
        logging.INFO,
        logger="orbit.backends.fsdp_utils.models.nemotron_h",
    ):
    ...
    torch.testing.assert_close(model.backbone[0].mixer.in_proj.weight, torch.tensor([[3.0]]))
    assert any(
        record.name == "orbit.backends.fsdp_utils.models.nemotron_h"
...
def test_weight_bridge_registry():
    ...
    from orbit.backends.fsdp_utils.adaptations.weight_bridge import get_param_transform, register_param_transform
...
def test_model_patch_registry_gating():
    ...
    from orbit.backends.fsdp_utils.adaptations.class_patches import _MODEL_PATCH_HOOKS
...
def test_model_type_verified_accepts_recorded_models(model_type, caplog):
    ...
    from orbit.backends.fsdp_utils.adaptations.class_patches import check_model_type_verified
...
def test_verified_model_types_match_recorded_validation():
    from orbit.backends.fsdp_utils.adaptations.class_patches import VERIFIED_MODEL_TYPES
...
def test_packed_seq_context_boundaries():
    ...
    from orbit.backends.fsdp_utils.adaptations.packing.boundaries import packed_seq_context
...
def test_nemotron_attention_reuses_precomputed_max_seqlen(monkeypatch):
    ...
    from orbit.backends.fsdp_utils.models import nemotron_h
...
def test_nemotron_pattern_to_list_repair():
    ...
    from orbit.backends.fsdp_utils.adaptations.specs.nemotron_h import _repair_pattern_to_list
...
def test_packing_registry():
    ...
    from orbit.backends.fsdp_utils.adaptations.packing import get_packing_patches
