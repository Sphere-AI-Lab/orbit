    ...
    pins.write_text(
        "\n".join(
            [
                ...
                "PYTORCH_INDEX_URL=https://example.test/torch",
                "FLASHINFER_INDEX_URL=https://example.test/flashinfer",
                "SGLANG_WHEEL_INDEX_URL=https://example.test/sglang",
                "SGLANG_SOURCE=https://example.test/sglang.git",
                f"SGLANG_COMMIT={'1' * 40}",
                "MEGATRON_LM_SOURCE=https://example.test/megatron.git",
                f"MEGATRON_LM_COMMIT={'2' * 40}",
                "MEGATRON_BRIDGE_SOURCE=https://example.test/bridge.git",
                f"MEGATRON_BRIDGE_COMMIT={'3' * 40}",
                "TRANSFORMER_ENGINE_SOURCE=https://example.test/te.git",
                f"TRANSFORMER_ENGINE_COMMIT={'4' * 40}",
                "APEX_SOURCE=https://example.test/apex.git",
                f"APEX_COMMIT={'5' * 40}",
            ]
        )
        + "\n"
