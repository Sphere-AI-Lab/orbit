...
def test_the_vendored_backend_init_no_longer_carries_orbits_import():
    """Upstream loads its OWN bridge plugins here; orbit's are a separate tree.

    ``miles_plugins.megatron_bridge`` is imported by the vendored file on this
    base -- that line is upstream's and stays. What must not come back is the
    orbit line next to it, which reached one level deeper into
    ``patches.bridges`` (orbit's own bridge subclasses, which upstream's
    ``__init__`` does not import).
    """
    src = (REPO / "miles" / "backends" / "megatron_utils" / "__init__.py").read_text()
    assert "patches.bridges" not in src
