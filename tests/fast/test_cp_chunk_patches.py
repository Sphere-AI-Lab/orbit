...
def _state(cp_size, cp_rank=0):
    """A ParallelState with every group trivial except CP.

    The required groups are spelled out from the dataclass rather than listed by
    hand: this miles base takes eight and has added to that set before.
    """
    trivial = parallel.GroupInfo(rank=0, size=1, group=None)
    cp = parallel.GroupInfo(rank=cp_rank, size=cp_size, group=None)
    fields = {
        name: trivial
        for name, field in parallel.ParallelState.__dataclass_fields__.items()
        if field.type is parallel.GroupInfo or field.type == "GroupInfo"
    }
    fields.update(intra_dp_cp=cp, cp=cp)
    return parallel.ParallelState(**fields)
