...
class _Engine:
    ...
    # this miles base brackets a weight update with begin/end calls on the engine
    begin_weight_update = _RemoteMethod()
    end_weight_update = _RemoteMethod()


class _Iterator:
    # this base passes weight_type= per call (it replaced the ctor-level is_lora flag)
    def get_hf_weight_chunks(self, _weights, **_kwargs):
...
def _make_updater(update_mod, transport, sync_spec):
    ...
    # this base still reads the legacy is_lora alongside peft_method
...
def test_distributed_peft_source_failure_reaches_peer_before_next_barrier(
    ...
    # on this base the PEFT chunk loop and _raise_distributed_peft_failure are lifted into
    # the home mixin, so get_gloo_group resolves in THAT module, not the vendored one
    import orbit.transport.update_weight_ext as update_ext_mod
    monkeypatch.setattr(update_ext_mod, "get_gloo_group", lambda: gloo_group)
    # post_process_weights is gone on this miles base -- upstream replaced that call, so
    # there is nothing to stub out here (see the comments in update_weight_ext.py)
    monkeypatch.setattr(update_ext_mod, "sum_metrics_across_ranks", lambda values, **_kwargs: values)
    monkeypatch.setattr(update_ext_mod, "emit_update_weights_metrics", lambda **_kwargs: None)
    monkeypatch.setattr(update_ext_mod, "emit_timeline_event", lambda *_args, **_kwargs: None)
