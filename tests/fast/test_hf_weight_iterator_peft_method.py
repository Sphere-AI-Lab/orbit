...
def test_create_stayed_in_the_vendored_class():
    """Only ``__init__`` moved to the mixin; ``create`` stays vendored.

    On this miles base ``create`` itself carries orbit's peft_method plumbing (it
    normalises the legacy ``is_lora`` and forwards the method), so unlike on
    orbit-main-isolated it does mention the name. What matters here is that it is
    still the vendored class's own method -- moving it too would put the factory
    behind the mixin and break subclass dispatch.
    """
    assert "create" in HfWeightIteratorBase.__dict__
    assert "peft_method" in HfWeightIteratorBase.create.__code__.co_varnames
    assert "__init__" not in HfWeightIteratorBase.__dict__, (
        "__init__ is back in the vendored body; its own body outranks the mixin "
        "in the MRO, so orbit's peft_method plumbing would never run"
    )
