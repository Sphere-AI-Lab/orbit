...
The bug the sanitiser exists for is silent: upstream's repair hands a token the
same expert twice, Megatron's dispatch map accepts it, and the model routes
somewhere it never chose. So every assertion below is on the ids that come out,
...
# A replayed route with one padded slot per token. Upstream repairs both with
# `arange(2) % 4` -> 0 and 1, which is precisely the id each token already has.
...
def test_upstream_alone_produces_the_duplicate_this_seam_removes():
