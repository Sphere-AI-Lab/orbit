...
class TestWaitForServerReadySimulatedDelays:
    ...
    def test_succeeds_after_simulated_delay(self, simulated_delay_s):
        ...
        with (
            patch("orbit.utils.http_utils.time.time", side_effect=fake_time_fn),
            patch("orbit.utils.http_utils.time.sleep", side_effect=fake_sleep),
            patch("orbit.utils.http_utils.socket.create_connection", side_effect=fake_connect),
        ):
