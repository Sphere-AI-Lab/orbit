...
import orbit.peft.utils.chat_template_utils as _orbit_templates
...
def test_ensure_sets_the_template(base_tokenizer):
    from orbit.peft.utils.llama3_chat_template import (
...
def test_template_date_is_a_literal_not_a_clock():
    ...
    from orbit.peft.utils.llama3_chat_template import LLAMA3_CHAT_TEMPLATE
...
def test_pinned_template_matches_source():
    ...
    assert (
        ...
        "by running: orbit/peft/utils/llama3_chat_template.py (extract and embed with raw string r\"\"\"...\"\"\")."
    )
...
def test_bundled_jinja_matches_the_pinned_python_constant():
    ...
    caller: `load_tokenizer` (orbit/utils/processing_utils.py) only ever sets
    ...
    `orbit/peft/utils/chat_template_utils/templates/llama3.1_pinned.jinja` (the file a
