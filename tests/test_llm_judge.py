"""LLM-judge reward hooks (miles/orbit/rewards/llm_judge.py).
...
from miles.orbit.rewards import llm_judge, scoring_client
...
def test_validate_judge_requires_base_url():
    args = argparse.Namespace(
        custom_rm_path="miles.orbit.rewards.llm_judge.reward_func", judge_base_url=None, judge_mode="equivalence"
    )
...
def test_validate_judge_noop_for_other_rm():
    args = argparse.Namespace(custom_rm_path="miles.orbit.opd.opd_sglang.reward_func", judge_base_url=None)
    _validate_judge_args(args)


def test_validate_judge_passes_when_configured():
    args = argparse.Namespace(
        custom_rm_path="miles.orbit.rewards.llm_judge.reward_func",
        judge_base_url="http://judge:30600",
        judge_mode="score",
    )
    _validate_judge_args(args)
