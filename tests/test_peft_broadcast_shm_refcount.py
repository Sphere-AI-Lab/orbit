...
def test_payload_survives_every_tp_rank_rebuild(adapter_tensors, tmp_path):
    ...
    assert alive == created, (
        "segments were unlinked while the producer still holds the tensors: "
        f"{sorted(created - alive)}"
    )
...
def test_late_rank_can_still_open_the_segment(adapter_tensors, tmp_path):
    ...
    for rank in range(3):
        ...
        assert done.returncode == 0, (
            f"rank {rank} could not open the broadcast segment: {done.stderr[-400:]}"
        )
...
def test_repeated_pushes_do_not_grow_dev_shm(tmp_path):
    ...
    for push in range(6):
        tensors = {
            f"lora_A.{i}": torch.arange(1024, dtype=torch.float32)
            for i in range(tensors_per_push)
        }
        ...
        assert len(mine()) <= tensors_per_push, (
            f"/dev/shm grew to {len(mine())} segments by push {push + 1}; "
            "the pre-paid increfs are leaking"
        )
