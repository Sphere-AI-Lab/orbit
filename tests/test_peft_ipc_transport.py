...
class _FailingEngine:
    def __init__(self):
        self.update_adapter_from_rank_tensors = _FailingRemoteMethod(RuntimeError("scheduler load failed"))
        self.update_weight_version = _RemoteMethod({"versioned": True})
...
class _FailingVersionEngine(_FakeEngine):
    def __init__(self):
        super().__init__()
        self.update_weight_version = _FailingRemoteMethod(RuntimeError("weight version failed"))
...
def test_ipc_oft_propagates_source_load_failure_across_engine_groups(monkeypatch, failure_point, expected_error):
    rank = {"value": 0}
...
def test_ipc_oft_propagates_failed_engine_result_to_peer_rank(monkeypatch):
    ...
    failed_engine.update_adapter_from_rank_tensors = _RemoteMethod({"success": False, "error": "adapter rejected"})
    source_backend = IpcBackend(
        args=Namespace(
            peft_method="oft",
            peft_distributed_transport="nccl",
