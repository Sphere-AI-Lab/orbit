...
def test_engine_serializes_ray_adapter_for_every_tp_rank_under_file_system(monkeypatch):
    ...
    TP1 indexes past it. Each payload must be serialized independently so its
    file-system shared-memory ownership credit has exactly one consumer.
    """
    calls = []
    ...
    # update_adapter_from_ray_tensor lives in the orbit/sglang/engine_ext.py home
    # mixin (Phase 3 slice 3c P2 move); patch its module-level names there.
    monkeypatch.setattr(engine_ext_module, "MultiprocessingSerializer", _Serializer)
    monkeypatch.setattr(
        engine_ext_module,
        "_balance_broadcast_shm_refcounts",
        lambda *_args: pytest.fail(
            "independently serialized TP payloads must not pre-pay broadcast references"
        ),
    )
    engine = SGLangEngine.__new__(SGLangEngine)
    engine.nnodes = 1
...
def test_engine_rejects_multi_node_ray_tensor_serialization(monkeypatch):
    ...
    engine.args = Namespace(num_gpus_per_node=4, rollout_num_gpus_per_engine=8)
    engine.num_gpus_per_engine = 8
    engine.update_weights_from_tensor = lambda **_kwargs: pytest.fail(
        "multi-node rejection must precede RPC dispatch"
    )
...
def test_cuda_ipc_propagates_source_load_failure_to_peer_rank(monkeypatch):
    ...
    failed_engine.update_weights_from_tensor = _FailingRemoteMethod(
        RuntimeError("scheduler load failed")
    )
    source_backend = IpcBackend(
        args=Namespace(
