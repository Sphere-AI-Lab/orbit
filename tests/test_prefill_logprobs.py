"""Prefill log-prob recompute (true-on-policy Phase 1, port of miles prefill_logprobs).

Rollout log-probs recomputed via one clean SGLang prefill pass (flush_cache +
max_new_tokens=0 scoring) instead of accumulated decode steps — removes
KV-cache / chunked-prefill / batch-composition variance from rollout_log_probs.
"""
...
async def test_recompute_rollout_logprobs_via_prefill_uses_response_tail(monkeypatch):
    ...
    args = SimpleNamespace(recompute_logprobs_via_prefill=True, peft_method="none")
    seen = {}
