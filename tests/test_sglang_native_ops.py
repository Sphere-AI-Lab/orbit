...
def test_compute_server_args_configures_oft_student_path(monkeypatch):
...
def test_compute_server_args_reserves_staging_slot_with_double_buffer(monkeypatch):
    kwargs = _compute_oft_server_args(
...
def test_compute_server_args_omits_peft_paths_without_adapter_path(monkeypatch):
    kwargs = _compute_oft_server_args(
        _oft_server_args(adapter_double_buffer=False, oft_adapter_path=None),
        monkeypatch,
    )

    assert "peft_paths" not in kwargs
