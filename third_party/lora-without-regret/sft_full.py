...
def parse_args():
    parser = argparse.ArgumentParser(description="Full fine-tune a language model")
    parser.add_argument(
        "--model-id",
        type=str,
    ...
    parser.add_argument(
        ...
        help="W&B project name (default: full-finetuning)",
    )
    parser.add_argument(
        "--wandb-run-name",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory to save the model (default: ./finetuned_model)",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def main():
    ...
    def eval(model, step=None):
        model.eval()
        total_loss = 0
        # ORBIT-LOCAL ADDITION (instrumentation only -- val_loss below is untouched).
        # See the identical block in sft_lora.py: Orbit reports a GLOBAL token-weighted
        # NLL, this script reports a mean over batches of per-batch token-means, and gate
        # G2 compares the two numbers directly. Denominator matches HF's own -- labels are
        # shifted internally, so position 0 never scores.
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                ...
                n_tokens = int((batch["labels"][:, 1:] != -100).sum().item())
                total_weighted_loss += loss.item() * n_tokens
                total_tokens += n_tokens
        val_loss = total_loss / len(val_dataloader)
        token_weighted_loss = (
            total_weighted_loss / total_tokens if total_tokens else float("nan")
        )
        print(f"val_loss: {val_loss:.4f} at step {step}")
        print(
            f"val_loss_token_weighted: {token_weighted_loss:.6f} "
            f"tokens: {total_tokens} batches: {len(val_dataloader)} at step {step}"
        )

        if use_wandb and step is not None:
            wandb.log(
                {"val/loss": val_loss, "val/loss_token_weighted": token_weighted_loss},
                step=step,
            )
