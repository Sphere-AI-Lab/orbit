...
def test_summarize_finds_run_dir_nested_under_campaign(tmp_path):
    # run_compare.py (~line 339) nests runs under
    # <output-dir>/<campaign>/<run_id>/ when --campaign is used; summarize()
    # must descend into a non-run-id child directory (the campaign) and scan
    # its immediate children as run dirs too.
    run_dir = tmp_path / "a1" / "r00_runtime_qwen3_4b_bf16_oft_async_g0123"
