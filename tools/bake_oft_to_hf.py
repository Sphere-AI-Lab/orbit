#!/usr/bin/env python
"""Bake a single miles OFT adapter into a standalone dense HF model.

Thin CLI over miles.merge.bake_hf.bake_hf_model for one adapter directory
...
paths that expect dense Hugging Face weights.
    ...
    python tools/bake_oft_to_hf.py \
        ...
        --adapter miles_ckpts/RUN/iter_0000059/adapter \
        --output baked/A1
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--block-size", type=int, default=None, help="OFT block size; default: oft_block_size from adapter_config.json"
    )
