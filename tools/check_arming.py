...
* Only static ``import``/``from`` statements. ``importlib.import_module(name)``
  with a computed name is invisible.
...
def patch_targets() -> set[str]:
    """Modules whose behaviour only exists once orbit is imported."""
    ...
    targets = {p.module for p in registry()}
    targets |= {name for name, _ in seam_registry()}
    return targets
...
def collect_errors() -> list[str]:
    ...
    for mod, path in sorted(paths.items()):
        ...
        errors.append(
            f"{rel}: {why} and reaches patched module(s) {', '.join(hit)} "
