...
def read_oft_config(adapter_dir: str) -> dict:
    ...
    if (cfg.get("peft_type") or "").upper() != "OFT":
        raise ValueError(f"adapter at {adapter_dir} has peft_type={cfg.get('peft_type')!r}, expected OFT")
    return cfg
...
def validate_adapters(adapter_dirs: list[str]) -> dict:
    ...
    for d, cfg in zip(adapter_dirs[1:], configs[1:], strict=True):
        for key in _COMPAT_KEYS:
            ...
            if not compatible:
                raise ValueError(f"adapter {d} differs on {key}: {got!r} != {expected!r}")
    return ref
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--device", default="cpu", help="device for the --save-hf bake (e.g. cuda:0); default cpu")
    ...
    if args.save_hf:
        ...
        if not base:
            raise ValueError(
                "--save-hf needs a base model: pass --base or ensure adapter_config has base_model_name_or_path"
            )
